...
| PL!N-bp4-030-L | Daydream Mermaid | 成功時: EDK1枚ウェイト or 控え室メンバー1枚回収（成功ライブ虹ヶ咲ありなら複数選択可・FAQ191） **2026-06-28: 虹なしでも1択発動** |
...
## 2026-07-29 修正（ブレード二重付与）

| ID | 内容 |
|----|------|
| PL!N-bp4-029-L | ライブ開始時（1ターン目のみ）の「虹ヶ咲メンバー1人がブレードを得る」が、常時セグメント再生成と直接付与の両方で適用されブレードが2個付いていた。`extractInlineLiveEndGrantJouji` にハート／ブレードアイコンのガードを追加 |
