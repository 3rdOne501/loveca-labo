...
## 2026-06-30 2回監修（メンバー 001–018）

`node scripts/verify-aqours-bp3.mjs` **22ケース**（メンバー16 + ライブ代表6 + 能力なし2）。

### 修正した

| ID | 名前 | 内容 |
|----|------|------|
| PL!S-bp3-001-P | 高海千歌 | 起動はセンターのみ → `kidouT` に `stageArea`/`stageAreas` を付与（006 善子も同様） |
| PL!S-bp3-005-P | 渡辺曜 | ライブ成功ドローに `requiresOwnYellRevealCountLessThanOpponent`（自エール公開枚数 < 相手） |
| PL!S-bp3-008-P | 小原鞠莉 | 回収ライブがスコア6+ Aqours のときのみ E4枚アクティブ → `energyOnPickedLiveFilters` |

### 問題なし（再確認）

002 梨子、003 果南、004 ダイヤ、007 花丸、009 ルビィ、010/011 アクティブ、012/017 相手ウェイト、016 花丸常時コスト+1（jouji `stage_cost_plus_per_success_live`）。

## 2026-06-30 2回監修（ライブ 019–025）

2026-06-28 修正（019/021/025）後の再確認。**新規コード修正なし**。

| ID | 結果 |
|----|------|
| 019 MIRACLE WAVE | OR 条件（全BH / 余剰ハート2+）→ スコア4  OK |
| 020 ダイスキ… | 自動・ターン1回・BH2枚以下再エール OK |
| 021 想いよ… | 控え室→山札上任意＋ブレード付与 OK |
| 022/023 | 能力なし OK |
| 024 Deep Resonance | センターAqours9+→BL2 or 相手ウェイト OK |
| 025 SUKI… | Aqours1人＋BL6+でスコア+1 OK |

## 2026-07-29 修正（起動コスト）

| ID | 内容 |
|----|------|
| PL!S-bp3-006 | 起動コストの「このメンバーをウェイトにし」を支払った後、ハンドラが重ねて自分を控え室へ送っていた（ウェイト≠控え室）。ハンドラの自分控え室送りを削除。対象は「このメンバー以外の『Aqours』のメンバー」に限定して提示 |
