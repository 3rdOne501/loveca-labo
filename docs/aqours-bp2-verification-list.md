...
## 2026-06-30 検証（メンバー・2回監修）

能力あり11枚（001–010, 016）: **guided_manual=0**。`verify-aqours-bp2.mjs` 22ケース通過。

### 修正した

| ID | 名前 | 内容 |
|----|------|------|
| PL!S-bp2-008-P | 小原鞠莉 | `stage_all_areas_grant_quoted` に全エリア Aqours 異名条件を適用。付与セグメントを `yell_reveal_live_score_tiered`（エール公開ライブ1枚+→+1、3枚+→+2）として動的スコア計算 |

### 問題なし（再確認）

001 jouji（自SL0・相手SL1+→ブレード3）、002–007、009–010、016。011–015・017–018 能力なし。

## 2026-06-30 検証（ライブ・2回監修）

019/020/026 能力なし。021–025 分類・前提条件・ハンドラ再確認。**修正なし**。

| ID | 確認内容 |
|----|----------|
| PL!S-bp2-021-L | エール公開ライブ1枚まで山札下（`pickType: ライブ`） |
| PL!S-bp2-022-L | `requiresDeckRefreshedThisTurn` + スコア+2 |
| PL!S-bp2-023-L | 他Aqoursライブ枠条件 + 全ステージメンバーへブレード1（FAQ Q121） |
| PL!S-bp2-024-L | 成功ライブ置き場不可（勝利時も控え室へ）+ 2ドロー→1枚捨て |
| PL!S-bp2-025-L | 自成功2枚+→メンバー1人選択でブレード2（ライブ終了まで） |
