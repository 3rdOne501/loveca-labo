...
## 2026-07-29 修正（部分実行とブレード二重付与）

| ID | 内容 |
|----|------|
| PL!SP-bp4-004 | 登場時の「カードを2枚引き、控え室のコスト4以下Liella!を空きエリアに登場」が、山札が2枚未満だとドローも登場も中止していた。総合ルール1.3.2に従い引ける分だけ引いて登場処理へ進むよう変更 |
| PL!SP-bp4-023-L | 指名メンバーと別のLiella!メンバーへのブレード付与が二重に適用されていた（`extractInlineLiveEndGrantJouji` のアイコンガード追加で解消） |
| PL!SP-bp4-024-L | 左サイドの heart02 3つ以上のメンバーへのブレード2個が二重（4個）に適用されていた。同上のガードで解消 |
