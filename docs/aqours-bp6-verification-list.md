...
## 2026-07-29 修正（コストと効果の切り分け）

| ID | 内容 |
|----|------|
| PL!S-bp6-003 | 分類器が共有分岐で `costSelfWait` を固定していたため、E2枚＋手札1枚が支払いコストのこのカードにも自ウェイトとセンター限定が要求されていた。コストはカード文（コスト部）から判定するよう変更 |
| PL!S-bp6-011-N | 「カードを2枚引き、手札を1枚控え室に置く」の後半をコスト（`handDiscardToWaiting`）扱いしていたため捨てが二重に。`effectDiscardCount`（効果本文）へ移動。「控え室から登場している場合」はコスト支払い前に判定 |
| PL!S-bp6-010-N | ライブ開始時のハート付与が、常時セグメント再生成と直接付与の両方で二重に適用されていた（`extractInlineLiveEndGrantJouji` のアイコンガード追加で解消） |
