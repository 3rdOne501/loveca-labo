...
| [x] | 004 | PL!N-bp1-004-R | 朝香果林 | activate_energy | ほかの虹ヶ咲 on stage→E1枚（minStageSeriesMembers:2） |
...
| [x] | 008 | PL!N-bp1-008-R | エマ・ヴェルデ | kidou_hand_cost_wait_pick_hand | 手札メンバー1捨→控え室からより低コストメンバー回収 |
...
| [x] | 012 | PL!N-bp1-012-R＋ | 鐘嵐珠 | jouji blade_conditional + kidou_wait_pick_hand | 常時：ライブ中3枚以上かつ虹ヶ咲ライブでALLハート2・ブレード2。起動EEE：控え室ライブ1枚手札 |
...
## 2026-06-30 再監修（2回目・メンバー 001–012）

| ID | 内容 |
|----|------|
| PL!N-bp1-004-R | `parseAbilityPickFilters`: ほかの『シリーズ』→ `minStageSeriesMembers:2`（横展開: Liella pb1-009 等） |
| PL!N-bp1-006-R＋ | kidou E2枚: `requiresSeriesEnteredThisTurn`（このターン登場の虹ヶ咲） |
| PL!N-bp1-008-R | `pickMaxCostBelowHandDiscarded` + `handDiscardMustBeMember`（手札捨て参照より低コストのみ回収） |
| （横展開） | verify 18ケース、`audit-common-patterns` 3ルール追加 |

## 2026-06-30 再監修（2回目・ライブ 025–029）

| ID | 内容 |
|----|------|
| （全体） | 分類・ハンドラ OK。実行時バグなし。verify 19ケース（025 能力なし追加） |
| PL!N-bp1-027-L | `distinct_heart_colors_on_stage_members`（実効ハート色、ALL除外 FAQ Q67）— 2026-06-28 修正済を再確認 |
| PL!N-bp1-028-L | 任意EE2 + `minStageSeriesMembers:1` |
| PL!N-bp1-029-L | `minLiveFrameCount:3`（ライブ中3枚以上） |
