...
## 2026-07-29 修正（ハート二重付与）

| ID | 内容 |
|----|------|
| PL!S-bp5-013-N | ライブ開始時の heart04 付与が、常時セグメント再生成と直接付与の両方で二重に適用されていた。`extractInlineLiveEndGrantJouji` にハート／ブレードアイコンのガードを追加 |
| PL!S-bp5-017-N | 同型（heart05 付与）。同上のガードで解消 |
