#!/usr/bin/env node
/** 虹ヶ咲 pb1（PL!N-pb1）効果検証リスト: カード文と分類の整合性監査 */
...
function repIdForNum(num) {
  const ids = Object.keys(cards).filter((k) => k.match(new RegExp(`PL!N-pb1-${num}-`)));
  return ids.find((k) => k.endsWith("-P")) || ids.find((k) => k.endsWith("-R")) || ids.find((k) => k.endsWith("-N")) || ids.find((k) => k.endsWith("-L")) || null;
}

/** @type {string[]} */
const errors = [];
const skipNoAbility = new Set(["025","026","027","029","030","031","032","033","040"]);
...
if (errors.length) {
  console.error(`${errors.length} niji-pb1 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-niji-pb1-text OK");
