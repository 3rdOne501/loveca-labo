#!/usr/bin/env node
/** Liella! bp2 / NEXTSTEP（PL!SP-bp2）: カード文と分類の整合性監査 */
...
function isLiellaBp2(id, card) {
  return /-bp2-/.test(id) && ((card.series || "").includes("スーパースター") || id.startsWith("PL!SP-bp2"));
}
...
for (const [id, card] of Object.entries(cards).sort(
  ...
  if (!isLiellaBp2(id, card)) continue;
...
if (errors.length) {
  console.error(`${errors.length} liella-bp2 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-liella-bp2-text OK");
