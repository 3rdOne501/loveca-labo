#!/usr/bin/env node
/** μ's pb1（PL!-pb1）効果検証リスト: カード文と分類の整合性監査 */
...
function repIdForNum(num) {
  const ids = Object.keys(cards).filter((k) => k.match(new RegExp(`PL!-pb1-${num}-`)));
  return ids.find((k) => k.endsWith("-P")) || ids.find((k) => k.endsWith("-R")) || ids.find((k) => k.endsWith("-N")) || ids.find((k) => k.endsWith("-L")) || null;
}

/** @type {string[]} */
const errors = [];
const skipNoAbility = new Set(["020","021","022","023","026","027","033"]);
...
if (errors.length) {
  console.error(`${errors.length} muse-pb1 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-muse-pb1-text OK");
