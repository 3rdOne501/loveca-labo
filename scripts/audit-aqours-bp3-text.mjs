#!/usr/bin/env node
/** Aqours bp3 / 夏、はじまる。（PL!S-bp3）: カード文と分類の整合性監査 */
...
function isAqoursBp3(id, card) {
  return /-bp3-/.test(id) && ((card.series || "").includes("サンシャイン") || id.startsWith("PL!S-bp3"));
}
...
for (const [id, card] of Object.entries(cards).sort(
  ...
  if (!isAqoursBp3(id, card)) continue;
...
if (errors.length) {
  console.error(`${errors.length} aqours-bp3 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-aqours-bp3-text OK");
