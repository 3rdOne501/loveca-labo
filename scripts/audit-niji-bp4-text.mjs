#!/usr/bin/env node
/** 虹ヶ咲 bp4 / SAPPHIREMOON（PL!N-bp4）: カード文と分類の整合性監査 */
...
function isNijiBp4(id, card) {
  return /-bp4-/.test(id) && ((card.series || "").includes("虹ヶ咲") || id.startsWith("PL!N-bp4"));
}
...
for (const [id, card] of Object.entries(cards).sort(
  ...
  for (const seg of splitAbilityByTriggers(raw)) {
    ...
    if (seg.trigger === "jouji") {
      ...
      if (j.kind === "jouji_manual" || j.template === "jouji_manual") {
        errors.push(`${id} jouji: manual`);
      }
      continue;
    }
...
if (errors.length) {
  console.error(`${errors.length} niji-bp4 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-niji-bp4-text OK");
