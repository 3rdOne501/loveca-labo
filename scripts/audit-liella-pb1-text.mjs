#!/usr/bin/env node
/** Liella! pb1（PL!SP-pb1）効果検証リスト: カード文と分類の整合性監査 */
...
function repIdForNum(num) {
  const ids = Object.keys(cards).filter((k) => k.match(new RegExp(`PL!SP-pb1-${num}-`)));
  return ids.find((k) => k.endsWith("-P")) || ids.find((k) => k.endsWith("-R")) || ids.find((k) => k.endsWith("-N")) || ids.find((k) => k.endsWith("-L")) || null;
}

/** @type {string[]} */
const errors = [];
const skipNoAbility = new Set(["012","013","014","019","022","026"]);
...
if (errors.length) {
  console.error(`${errors.length} liella-pb1 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-liella-pb1-text OK");
