#!/usr/bin/env node
/** 蓮ノ空 bp6（PL!HS-bp6）効果検証リスト: カード文と分類の整合性監査 */
...
function repIdForNum(num) {
  const ids = Object.keys(cards).filter((k) => k.match(new RegExp(`PL!HS-bp6-${num}-`)));
  return ids.find((k) => k.endsWith("-P")) || ids.find((k) => k.endsWith("-R")) || ids.find((k) => k.endsWith("-N")) || ids.find((k) => k.endsWith("-L")) || null;
}

/** @type {string[]} */
const errors = [];
const skipNoAbility = new Set(["021","023","024","026"]);
...
if (errors.length) {
  console.error(`${errors.length} hasunosora-bp6 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-hasunosora-bp6-text OK");
