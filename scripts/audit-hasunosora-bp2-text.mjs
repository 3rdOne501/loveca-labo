#!/usr/bin/env node
/** 蓮ノ空 bp2 / NEXTSTEP（PL!HS-bp2）: カード文と分類の整合性監査 */
...
function isHasunosoraBp2(id, card) {
  return /-bp2-/.test(id) && ((card.series || "").includes("蓮ノ空") || id.startsWith("PL!HS-bp2"));
}
...
for (const [id, card] of Object.entries(cards).sort(
  ...
  if (!isHasunosoraBp2(id, card)) continue;
...
if (errors.length) {
  console.error(`${errors.length} hasunosora-bp2 issue(s):`);
  errors.forEach((e) => console.error("  FAIL", e));
  process.exit(1);
}
console.log("audit-hasunosora-bp2-text OK");
