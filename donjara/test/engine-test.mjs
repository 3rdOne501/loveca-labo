...
check("バラバラ手は非和了", !analyzeHand(h5, cat).isAgari);
...
check("テンパイ判定", isTenpai(t13, cat));
check("待ちに muse-c1 が含まれる（単騎）", winningTiles(t13, cat).includes("muse-c1"));
...
const a7 = analyzeHand(concealed, cat);
...
const a8 = analyzeHand(seven, cat);
