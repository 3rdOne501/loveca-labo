...
/** 直近タップ時刻（連打速度 → シャカパチ速度） */
var shakaTapTimes = [];
var SHAKA_TAP_WINDOW_MS = 900;

/**
 * スリーブ同士を叩く「シャカパチ」合成音。
 * 連打が速いほど再生が短く・高く・密になる。
 */
export function playShakaPachiTap() {
  try {
    var AC = window.AudioContext || window.webkitAudioContext;
    if (!AC) return 0;
    if (!audioCtx) audioCtx = new AC();
    ...
    shakaTapTimes.push(nowMs);
    while (shakaTapTimes.length && nowMs - shakaTapTimes[0] > SHAKA_TAP_WINDOW_MS) {
      shakaTapTimes.shift();
    }

    var rate = 0;
    if (shakaTapTimes.length >= 2) {
      var span = nowMs - shakaTapTimes[0];
      if (span > 1) rate = (shakaTapTimes.length - 1) / (span / 1000);
    }
    // 0〜約12 taps/s を 0〜1 に正規化
    var intensity = Math.max(0, Math.min(1, rate / 12));

    var a = audioCtx;
    var t0 = a.currentTime;
    // 遅い: ~55ms / 速い: ~22ms
    var dur = 0.055 - intensity * 0.033;
    var len = Math.max(32, Math.floor(a.sampleRate * dur));
    ...
    src.buffer = buf;
    // 連打が速いほどピッチが上がる（再生速度）
    src.playbackRate.value = 0.85 + intensity * 1.35;

    var filt = a.createBiquadFilter();
    filt.type = "bandpass";
    filt.frequency.value = 900 + intensity * 2200;
    filt.Q.value = 0.7 + intensity * 0.8;

    var gain = a.createGain();
    var vol = 0.22 + intensity * 0.2;
    ...
    // とても速い連打では短い二重タップで「パチパチ」感を足す
    if (intensity > 0.55) {
      var src2 = a.createBufferSource();
      src2.buffer = buf;
      src2.playbackRate.value = 1.1 + intensity * 1.2;
      var filt2 = a.createBiquadFilter();
      filt2.type = "highpass";
      filt2.frequency.value = 1400 + intensity * 1800;
      var gain2 = a.createGain();
      var t1 = t0 + 0.012 + (1 - intensity) * 0.02;
      gain2.gain.setValueAtTime(vol * 0.55, t1);
      gain2.gain.exponentialRampToValueAtTime(0.0001, t1 + dur * 0.7);
      src2.connect(filt2);
      filt2.connect(gain2);
    ...
    try {
      if (typeof navigator !== "undefined" && typeof navigator.vibrate === "function") {
        navigator.vibrate(intensity > 0.6 ? 6 : 10);
      }
    } catch (_) {
      /* noop */
    }
