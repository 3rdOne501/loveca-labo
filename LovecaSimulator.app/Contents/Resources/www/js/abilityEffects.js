...
import { catalogCardGroupKey, catalogCardMatchesGroupTag, normalizeQuotedSeriesTag } from "./cardGroups.js";
 ...
 *   |'kidou_reveal_live_wait_pick_name_contains'
 *   |'kidou_energy_under_pick_waiting_hand'
 *   |'kidou_hand_cost_wait_pick_hand'
 *   |'kidou_hand_discard_activate_wait_opp_bonus'
 *   |'kidou_wait_to_stage'
 *   |'deck_top_to_waiting'
 *   |'deck_top_peek_optional_wait'
 *   |'deck_top_look_reorder'
 *   |'deck_top_pick_recover'
 *   |'deck_top_pick_enter_or_hand'
 *   |'toujou_wait_pick_hand'
 *   |'toujou_optional_pay_wait_recover_grant'
 *   |'toujou_optional_disable_live_start_recover_wait'
 ...
 *   |'deck_mill_conditional_grant'
 *   |'deck_mill_conditional_need_heart_reduce'
 ...
 *   |'live_start_optional_energy_blade_per_live_frame'
 ...
 *   |'toujou_draw_per_energy_unit'
 *   |'toujou_named_stage_activate_recover_wait'
 *   |'waiting_to_deck_bottom'
 *   |'grant_jouji_session'
 *   |'live_start_yell_reveal_reduction'
 *   |'live_start_position_change'
 *   |'kidou_deck_top_wait_position_change'
 ...
 *   |'optional_energy_card_score_plus_per_unit'
 ...
 *   |'live_success_surplus_heart_slot_draw'
 ...
 *   |'live_start_deck_top_if_all_members_grant'
 ...
 *   |'live_start_per_series_member_deck_look_reveal_score'
 ...
 *   |'toujou_draw_discard_if_from_waiting'
 ...
 *   |'live_start_score_plus_per_named_success_live'
 *   |'live_start_score_plus_per_success_live'
 ...
 *   |'toujou_baton_both_keep_hand_shuffle_deck_bottom_draw'
 ...
 * @property {number[]} [costsAnyOf] 印刷コストがいずれかに一致（例: 10か20）
 ...
 * @property {boolean} [requiresAllStageMembersActive] ステージの全メンバーがアクティブ（ウェイトでない）
 * @property {{smaller: string, larger: string} | null} [requiresNamedMemberPairCostOrder] 指名2人が登場し、largerのコスト>smallerのコスト
 * @property {Array<{area: 'left'|'center'|'right', name: string}> | null} [requiresStageNamedMemberAreas] 各エリアに指名メンバーが登場
 * @property {string | null} [requiresStageOnlySeries] ステージのメンバーが指定シリーズのみの場合
 * @property {string | null} [requiresOtherSeriesLiveOnFrameExcludeName] ライブ置き場に同名以外の指定シリーズライブがある
 * @property {string | null} [requiresOtherSeriesLiveOnFrameTag] 上記のシリーズタグ
 * @property {boolean} [requiresDeckRefreshedThisTurn] このターンにデッキがリフレッシュしていた
 * @property {string[] | null} [seriesTagsAny] いずれかのシリーズに一致
 * @property {number | null} [minDistinctSeriesMemberNames] ステージに名前の異なる指定シリーズメンバー数
 * @property {number | null} [minDistinctNamedStageMembersFromList] 指名候補のうちステージにいる名前の異なるメンバー数
 * @property {number | null} [minDistinctStageMemberNames] 自ステージの名前の異なるメンバー数下限
 * @property {string[] | null} [namedStageMemberList] 上記の指名候補（「」で列挙）
 * @property {number | null} [minNamedStageMemberFromList] 指名リストのいずれかがステージにいる（1=OR）
 ...
 * @property {number | null} [minYellRevealedSeriesMemberCount] エール公開に指定シリーズのメンバーカード枚数（同名可）
 * @property {string | null} [yellRevealedSeriesMemberTag]
 * @property {number | null} [minScore] ライブカードのスコア下限
 * @property {boolean} [requiresZeroSurplusHearts] このターン余剰ハート0（総合ルール 8.x 余剰ハート）
 * @property {AbilityPickFilters[]} [pickFilterAlternatives] いずれかを満たせばよい OR フィルタ
 * @property {boolean} [requiresHasBladeHeart] ブレードハートを持つカードのみ
 * @property {boolean} [requiresNoBladeHeart] ブレードハートを持たないカードのみ
 * @property {boolean} [requiresNoBladeHeart] ブレードハートを持たないカードのみ（山札見→手札等）
 * @property {boolean} [requiresOtherStageMemberMovedThisTurn] このターンに他のステージメンバーがエリア移動済み
 * @property {number | null} [minSurplusHearts] 余剰ハート数の下限
 * @property {number | null} [minStageAreaMemberBlade] 指定エリアのメンバーが持つブレード数の下限
 * @property {number | null} [minStageMemberBladeSum] ステージメンバーのブレード合計下限
 * @property {number | null} [forbidStageMemberMinBlade] 指定シリーズでこのブレード数以上のメンバーがいないこと
 * @property {string | null} [forbidStageMemberMinBladeSeries] 上記のシリーズタグ
 * @property {boolean} [requiresLiveFrameNoStartSuccessAbility] ライブ枠に LS/LS成功能力なしのカードがある
 ...
 * @property {number | null} [minYellRevealedNoBladeHeartMembers] エール公開にBHなしメンバーがN枚以上
 * @property {number | null} [minYellRevealedHeartColorKinds] エール公開カードの heart01–06 がN色種類以上
 * @property {boolean} [requiresYellRevealedOwnLiveCard] エール公開に自分のライブカードが1枚以上
 * @property {string | null} [requiresYellRevealedAllSeriesTag] エール公開の自分のカードがすべて指定シリーズ
 ...
 * @property {string | null} [minLiveFrameSeriesTag] minLiveFrameCount 時のシリーズ絞り
 ...
 * @property {'top'|'bottom'} [deckMillFrom] deckTopCount 枚を上または下からミル
 * @property {string | null} [millRequireAllSeriesMembers] ミルしたカードがすべて指定シリーズのメンバー
 * @property {string | null} [millRequireSeriesMember] ミル1枚が指定シリーズのメンバー（「それが…の場合」）
 * @property {boolean} [millRequireLive] ミル1枚がライブカード
 * @property {number} [millRequireMemberMinCost] ミル1枚がコストN以上のメンバー
 * @property {number} [millRequireDistinctBladeHeartColors] ミルしたメンバーのBH色（1–6）がN種類以上
 * @property {boolean} [millActivateSelf] 条件成立時にこのメンバーをアクティブにする
 * @property {number} [handDiscardToWaiting]
 * @property {boolean} [costHandDiscardAll] コスト: 手札をすべて控え室へ（任意）
 * @property {string | null} [costHandDiscardPickType] 手札コストで選ぶカード種（T_LIVE 等）
 * @property {number} [energyToDeckCount] 効果: エネルギー置き場→エネルギーデッキ（起動本文）
 * @property {boolean} [pickAny] 控え室回収の pickType 不問（任意カード）
 * @property {number} [deckDrawCount]
 * @property {number} [energyPerDrawUnit] 自分のエネルギーN枚につき deckDrawCount 枚引く（登場時）
 * @property {boolean} [allowOccupiedStageColumn] 登場時: 既にメンバーがいる列への手札登場を許可
 * @property {boolean} [grantPlayerCannotLiveUntilLiveEnd] 登場等: ライブ終了時まで自分はライブできない
 ...
 * @property {boolean} [costHandDiscardOptional] 必須ウェイト＋任意手札捨て（「ウェイトにし、手札…置いてもよい」）
 * @property {number | null} [oppWaitExactPrintedBlade] 相手ウェイト対象の元々ブレードがちょうどN
 ...
 * @property {boolean} [energySkipNextActivate] 置いたエネルギーは次のターンのアクティブフェイズにアクティブしない
 * @property {boolean} [requiresMoreEnergyThanOpponent] 自分のエネルギーが相手より多い場合のみ実行
 * @property {number | null} [minEnergyAdvantageOverOpponent] 自分のエネルギーが相手より N 枚以上多い場合のみ
 * @property {boolean} [costEnergyToDeck] コスト: エネルギー置き場のエネルギーをエネルギーデッキに置く
 * @property {number} [costEnergyToDeckCount] 上記コストの枚数
 * @property {boolean} [costEnergyUnderSelf] コスト: エネルギー置き場のエネルギーをこのメンバーの下に置く
 * @property {number} [costEnergyUnderSelfCount] 上記コストの枚数
 * @property {boolean} [requiresEnergyReturnedToDeckThisTurn] このターン置き場→エネルギーデッキが発生している場合のみ
 ...
 * @property {boolean} [requiresSelfMovedThisTurn]
 * @property {boolean} [costEnergyVariable]
 * @property {number} [optionalEnergyMaxPay]
 * @property {number} [bladeGainPerEnergy]
 ...
 * @property {boolean} [requiresBatonFromLowerCostMember] 低コストメンバーからのバトンタッチ必須（シリーズ不問）
 * @property {boolean} [requiresBatonFromNoAbilityMember] 能力なしメンバーからのバトンタッチ必須
 ...
 * @property {number | null} [bladeGainPerMoved] 控え室→デッキ下で1枚につき付与するブレード
 * @property {number | null} [energyActivePerMoved] 控え室→デッキ下で1枚につきアクティブにするエネルギー
 * @property {boolean} [millBothPlayers] deck_top_to_waiting: 相手も同数を山札上から控え室へ
 * @property {string | null} [handDiscardSeriesTag] 手札捨て対象のシリーズタグ
 * @property {number | null} [handDiscardNoBladeHeartMax] BHなし手札メンバーを最大N枚まで捨てる任意コスト
 * @property {boolean} [recoverCountMatchesDiscarded] 捨てた枚数と同数を控え室から回収
 * @property {number | null} [recoverCount] 控え室から回収する枚数
 * @property {AbilityPickFilters | null} [recoverFilters] 控え室回収のフィルタ
 * @property {string | null} [disableTargetSeriesTag] 登場時: 指定シリーズのメンバーのライブ開始時能力を無効化
 ...
 * @property {boolean} [waitingToDeckExact] 控え室→デッキでちょうど N 枚必要（不足時は不可）
 ...
 * @property {boolean} [bladePerDistinctNameUnder] 下の名前の異なるメンバー1枚につき bladeGain
 * @property {string | null} [millRequireSeriesTag] ミル条件: シリーズのライブ or 非BHメンバー
 ...
 * @property {boolean} [grantToEnteredMembersThisTurn] このターンに登場したステージメンバーへ付与
 * @property {string} [grantExcludeSeriesTag] 付与対象から除外するシリーズ（『X』以外）
 ...
 * @property {number[]} [scoreHeartColorSlots] ステージメンバーのハート色（1–6）のうち存在する色数
 * @property {boolean} [scorePlusOrPreconditions] ライブ成功時スコア＋の OR 複合条件（LL-bp5-001-L 等）
 ...
 * @property {string | null} [grantToCenterSeriesTag] センターエリアの指定シリーズメンバー全員へ付与
 * @property {boolean} [grantExcludeSelf] 付与先から能力発動元メンバーを除外
 ...
 * @property {boolean} [grantToAllStageMembers] ステージの全メンバーへ付与
 * @property {boolean} [optionalWaitingMemberToDeckTop] 任意: 控え室メンバー1枚を山札上に置いてから付与
 * @property {number} [minPickedMemberBlade] 選択メンバーのブレード下限（スコア加算等）
 * @property {boolean} [requiresStrictlyMostBladesBothStages] 選択メンバーが両ステージの他全員より多いブレード
 * @property {number} [minEnergyCountAfterMove] エネルギー移動後の置き場枚数下限（スコア付与）
 * @property {number} [minMovedEnergyCount] 移動したエネルギー枚数下限（スコア付与）
...
function isInlineTriggerTypeReference(raw, tokenStart, tokenEnd) {
  ...
  var beforePlain = String(raw.slice(Math.max(0, tokenStart - 12), tokenStart) || "")
    .replace(/\{\{[^}]+\}\}/g, "")
    .replace(/\s+/g, "");
  // 「その中から{{live_start}}能力を持たない…」— 能力種別フィルタのインライン参照（bp6-004 等）
  if (/から$/.test(beforePlain) && /^能力を持たない/.test(afterPlain)) return true;
  if (!/^能力/.test(afterPlain)) return false;
  if (/^能力も/.test(afterPlain)) return true;
  if (/^能力を持たない/.test(afterPlain)) return false;
...
/** Wiki トークン除去後も segRaw 上の icon_blade 等でブレード参照を検出 */
function abilityReferencesPrintedBlade(segRaw) {
  return /icon_blade/i.test(String(segRaw || ""));
}

/** 効果条件「ステージの左/右/センターエリアにいるメンバー」（配置制約とは別） */
function parseMemberLocationStageArea(segRaw) {
  var s = String(segRaw || "");
  if (/左サイドエリア/.test(s)) return "left";
  if (/右サイドエリア/.test(s)) return "right";
  if (/センターエリア/.test(s)) return "center";
  return parseStageAreaConstraint(s);
}
...
/**
 * 山札の上N枚見る → 手札に加える → 残り控え室（「公開」なしの 1枚/複数枚 も含む）。
 * @param {string} p
 * @param {string} segRaw
 * @param {Partial<ClassifiedAbility>} [extraPatch]
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckTopPickToHandPatch(p, segRaw, extraPatch) {
  ...
  if (/デッキの上に置/.test(p) && /好きな枚数/.test(p)) return null;
  if (
    /デッキの上からカードを(\d+)枚控え室に置/.test(p) &&
    /その後/.test(p) &&
    /控え室から.*手札に加/.test(p)
  ) {
    return null;
  ...
  var maxPickM =
    p.match(/(\d+)枚まで(?:公開して)?手札に加/) || p.match(/(\d+)枚まで公開して手札に加/);
  if (maxPickM) {
    patch.deckTopPickMax = Number(maxPickM[1]) || 1;
  } else if (/その中から1枚を手札に加え/.test(p)) {
    patch.deckTopPickMax = 1;
  } else {
    var pickCardM = p.match(/カードを(\d+)枚手札に加/);
    if (pickCardM) patch.deckTopPickMax = Number(pickCardM[1]) || 1;
    else if (/公開/.test(p)) patch.deckTopPickMax = 1;
    else return null;
  }

  if (/各グループ名につき1枚ずつ/.test(p)) {
    patch.deckTopPickDistinctGroup = true;
  }
...
/**
 * 「山札の上N枚見る。そのカードを控え室に置いてもよい」（peek 後任意控え室）。
 * @param {string} p
 * @param {Partial<ClassifiedAbility>} [extraPatch]
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckTopPeekOptionalWaitPatch(p, extraPatch) {
  var td = parseDeckTopCount(p);
  if (td == null || !/見る/.test(p) || !/控え室/.test(p)) return null;
  if (/その中から/.test(p) && /手札に加/.test(p)) return null;
  if (/1枚までをデッキの上に置/.test(p)) return null;
  return Object.assign({}, extraPatch || {}, {
    template: "deck_top_peek_optional_wait",
    deckTopCount: td,
    optional: /もよい/.test(p),
  });
}
...
/**
 * コスト部（「：」より前）から手札→控え室枚数を読む。
 * @param {string} costPart
 * @returns {number|null}
 */
function parseHandDiscardCostFromCostPart(costPart) {
  if (!costPart) return null;
  if (/手札をすべて控え室/.test(costPart)) return null;
  if (/手札のライブカードを1枚控え室/.test(costPart)) return 1;
  var m = costPart.match(/手札を(\d+)枚控え室に置/);
  if (m) return Number(m[1]) || null;
  if (/手札を1枚控え室に置/.test(costPart)) return 1;
  m = costPart.match(/カード(\d+)枚を控え室に置/);
  if (m) return Number(m[1]) || null;
  return null;
}

/**
 * 任意コストの手札捨て（ライブ1枚 / 全捨て）を効果パッチへ載せ、
 * 山札見・ドロー等の effect filters からコスト由来 pickType を除去する。
 * @param {Partial<ClassifiedAbility>} base
 * @param {Partial<ClassifiedAbility>} patch
 * @param {string} p
 * @returns {Partial<ClassifiedAbility>}
 */
function enrichOptionalHandDiscardCost(base, patch, p) {
  if (!patch) return patch;
  /** @type {Partial<ClassifiedAbility>} */
  var out = Object.assign({}, patch);
  if (base && base.filters) {
    out.filters = mergeAbilityPickFilters(base.filters, patch.filters || {});
  }
  if (base && base.costHandDiscardAll) {
    out.costHandDiscardAll = true;
    out.hasOptionalCost = true;
    out.optional = true;
  }
  if (
    base &&
    base.costHandDiscardPickType === T_LIVE &&
    base.handDiscardToWaiting &&
    base.handDiscardToWaiting > 0
  ) {
    out.handDiscardToWaiting = base.handDiscardToWaiting;
    out.costHandDiscardPickType = base.costHandDiscardPickType;
    out.hasOptionalCost = base.hasOptionalCost;
    out.optional = base.optional;
    if (out.filters && out.filters.pickType === T_LIVE) {
      var costPart = p.indexOf("：") >= 0 ? p.split("：")[0] : "";
      var effectPart = p.indexOf("：") >= 0 ? p.split("：").slice(1).join("：") : p;
      if (
        /手札のライブカードを1枚控え室/.test(costPart) &&
        (/デッキの上から|見る/.test(effectPart) ||
          /カードを(\d+)枚引/.test(effectPart) ||
          /好きなハート/.test(effectPart))
      ) {
        out.filters = Object.assign({}, out.filters, { pickType: null });
      }
    }
  }
  return out;
...
/** 相手ステージのウェイト対象エリア（右/左サイド等） @param {string} p @returns {Array<'left'|'center'|'right'>} */
function parseOppWaitStageAreas(p) {
  var s = String(p || "");
  /** @type {Array<'left'|'center'|'right'>} */
  var areas = [];
  if (/右サイドエリア.*左サイドエリア|左サイドエリア.*右サイドエリア/.test(s)) {
    areas.push("left", "right");
  } else if (/右サイドエリア/.test(s) && !/センターエリア/.test(s)) {
    areas.push("right");
  } else if (/左サイドエリア/.test(s) && !/センターエリア/.test(s)) {
    areas.push("left");
  } else if (/センターエリア/.test(s) && !/右サイド|左サイド/.test(s)) {
    areas.push("center");
  }
  return areas;
}

/** @param {string} p @returns {Partial<ClassifiedAbility>} */
function buildOppWaitStageMeta(p) {
  var oppCostMaxM = p.match(/コスト(\d+)以下/);
  var oppCostMinM = p.match(/コスト(\d+)以上/);
  var oppCntM = p.match(/(\d+)人までウェイト/);
  var bladeMax = parseOppWaitPrintedBladeLimit(p);
  var bladeExact = parseOppWaitExactPrintedBlade(p);
  var areas = parseOppWaitStageAreas(p);
  /** @type {Partial<ClassifiedAbility>} */
  var meta = {
  ...
  if (bladeExact != null) {
    meta.oppWaitExactPrintedBlade = bladeExact;
    meta.oppWaitMaxCost = 99;
  } else if (oppCostMaxM) meta.oppWaitMaxCost = Number(oppCostMaxM[1]);
  else if (oppCostMinM) {
    meta.oppWaitMinCost = Number(oppCostMinM[1]);
    meta.oppWaitMaxCost = 99;
  } else {
    meta.oppWaitMaxCost = bladeMax != null ? 99 : 4;
  }
...
function textHasHeartColorPickGrant(p) {
  if (/デッキの上からカードを\d+枚公開/.test(p) && /指定した色/.test(p)) return false;
  return (
...
function enrichHeartColorPickGrantPatch(p, segRaw, basePatch) {
  ...
  var handSeriesDiscM = p.match(/手札の『([^』]+)』のカードを(\d+)枚控え室に置いてもよい/);
  if (handSeriesDiscM) {
    patch.optional = true;
    patch.hasOptionalCost = true;
    patch.handDiscardExact = Number(normalizeFwDigits(handSeriesDiscM[2])) || 2;
    patch.handDiscardSeriesTag = handSeriesDiscM[1];
  }
  if (/手札のライブカードを1枚控え室に置いてもよい/.test(p)) {
    patch.optional = true;
    patch.hasOptionalCost = true;
    patch.handDiscardToWaiting = 1;
    patch.costHandDiscardPickType = T_LIVE;
  }
  var heartCountM = p.match(/選んだハートを([０-９\d]+)つ得る/);
  if (!heartCountM) heartCountM = p.match(/そのハートを([０-９\d]+)つ得る/);
  if (heartCountM) patch.grantHeartSlotCount = Number(normalizeFwDigits(heartCountM[1])) || 2;
 ...
 */
/**
 * 「右/左サイド・センターエリアに「名前」が登場」の並びを抽出（みらくりえーしょん 等）。
 * @param {string} p
 * @returns {Array<{area: 'left'|'center'|'right', name: string}> | null}
 */
function parseStageNamedMemberAreaRequirements(p) {
  var txt = String(p || "");
  /** @type {Array<{area: 'left'|'center'|'right', name: string}>} */
  var reqs = [];
  var re = /(左サイド|右サイド|センター)エリアに「([^」]+)」が/g;
  var m;
  while ((m = re.exec(txt)) !== null) {
    var area = m[1] === "左サイド" ? "left" : m[1] === "右サイド" ? "right" : "center";
    reqs.push({ area: area, name: m[2] });
  }
  return reqs.length ? reqs : null;
}

function parseNeedHeartReduceConditionFilters(p, segRaw) {
  var txt = String(p || "");
  var blob = txt + String(segRaw || "");
  ...
  var bladeTotalM =
    txt.match(/ステージ[^。]*ブレード[^。]*合計が([０-９\d]+)以上/) ||
    (/icon_blade/i.test(blob) && txt.match(/ステージ[^。]*持つ[^。]*合計が([０-９\d]+)以上/));
  if (bladeTotalM) f.requiresStageBladeTotal = Number(normalizeFwDigits(bladeTotalM[1])) || 0;
  if (/ステージにいるすべてのメンバーがアクティブ/.test(txt)) f.requiresAllStageMembersActive = true;
  return f;
}

/**
 * デッキ下ミル→条件成立時に必要ハートを減らす（ライブ開始時）。
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckMillConditionalNeedHeartReduce(p, segRaw) {
  var botMillM = p.match(/デッキの下からカードを(\d+)枚控え室に置/);
  var millN = 0;
  if (botMillM) millN = Number(normalizeFwDigits(botMillM[1])) || 1;
  else if (/デッキの一番下のカードを控え室に置く/.test(p)) millN = 1;
  else return null;
  if (!/必要ハート(?:を|は)/.test(p) || !/(減らす|少なくなる|減る)/.test(p)) return null;
  var seriesOneM = p.match(/それが『([^』]+)』のメンバーカードの場合/);
  var seriesAllM = p.match(/それらがすべて『([^』]+)』のメンバーカードの場合/);
  if (!seriesOneM && !seriesAllM) return null;
  var reduceMap = parseNeedHeartReduceFixedMap(segRaw);
  if (!Object.keys(reduceMap).length) return null;
  /** @type {Partial<ClassifiedAbility>} */
  var patch = {
    template: "deck_mill_conditional_need_heart_reduce",
    deckMillFrom: "bottom",
    deckTopCount: millN,
    needHeartReduceMap: reduceMap,
    requiresOnStage: true,
  };
  if (seriesOneM) patch.millRequireSeriesMember = normalizeQuotedSeriesTag(seriesOneM[1]);
  else if (seriesAllM) patch.millRequireAllSeriesMembers = normalizeQuotedSeriesTag(seriesAllM[1]);
  return patch;
}
...
/**
 * ステージのシリーズメンバーが持つハート色（列挙スロット）ごとにスコア＋N（Solitude Rain 等）。
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyLiveScorePerHeartColorOnStage(p, segRaw) {
  if (!/のうち1色につき/.test(p) || !/このカードのスコアを[＋+]/.test(normalizeFwDigits(p))) return null;
  if (!/ステージ/.test(p)) return null;
  var per = parseLiveCardScorePlusFromText(normalizeFwDigits(p)) || 1;
  /** @type {number[]} */
  var slots = [];
  var re = /\{\{heart_0?(\d)[^}]*\}\}/gi;
  var m;
  while ((m = re.exec(String(segRaw || p))) !== null) {
    var slot = Number(m[1]);
  ...
  return {
    template: "live_card_score_plus_per_unit",
    cardScorePerUnit: per,
    scoreUnitKind: "distinct_heart_colors_on_stage_members",
    scoreUnitSeries: seriesM ? seriesM[1] : null,
    scoreHeartColorSlots: slots,
    filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p)),
  };
}
...
/**
 * 「必要ハートを〜増やす」の増加分を {slotKey: count} で抽出（減算なしの単純増加）。
 * @param {string} segRaw
 * @returns {Object.<string, number>}
 */
function parseNeedHeartIncreaseFixedMap(segRaw) {
  var s = String(segRaw || "");
  var idx = s.indexOf("必要ハート");
  if (idx < 0) return {};
  var rest = s.slice(idx);
...
function parseGrantJoujiMeta(p, segRaw) {
  ...
  var centerSeriesGrantM = p.match(/センターエリアにいる『([^』]+)』のメンバー/);
  if (centerSeriesGrantM) {
    meta.grantToCenterSeriesTag = centerSeriesGrantM[1];
    meta.stageArea = "center";
  } else if (/センターエリアにいるメンバーは/.test(p)) {
    meta.grantToCenterMember = true;
  }

  if (/このメンバー以外の(?:『[^』]+』の)?メンバー1人/.test(p)) {
    meta.grantPickStageMembersMax = 1;
    meta.grantExcludeSelf = true;
  }
  var otherExclSeriesM = p.match(/このメンバー以外の『([^』]+)』のメンバー1人は/);
  if (otherExclSeriesM) {
    meta.grantToStageSeriesTag = otherExclSeriesM[1];
  ...
  if (stageGrantM) {
    ...
    /**
     * 「ステージにいる{{icon_blade}}をNつ以上持つ『シリーズ』のメンバー1人は」
     * wiki アイコン除去後は「ステージにいるをNつ以上持つ『…』のメンバー1人は」になる。
     */
    var stageBladeHoldGrantM = p.match(
      /ステージにいるを([０-９\d]+)つ以上持つ『([^』]+)』のメンバー1人は/,
    );
    if (
      stageBladeHoldGrantM &&
      /\{\{[^}]*blade[^}]*\}\}/i.test(String(segRaw || "").split(/つ以上持つ/)[0] || "")
    ) {
      meta.grantToStageSeriesTag = stageBladeHoldGrantM[2];
      meta.grantToStageSeriesMax = 1;
      meta.minPickedMemberBlade = Number(normalizeFwDigits(stageBladeHoldGrantM[1])) || 0;
    } else {
    var stageCostGrantM = p.match(/ステージにいるコスト([０-９\d]+)以上の『([^』]+)』のメンバー1人は/);
    if (stageCostGrantM) {
      meta.grantToStageSeriesTag = stageCostGrantM[2];
      meta.grantToStageSeriesMax = 1;
    } else {
    ...
    var stageAllSeriesM = p.match(/(?:自分の)?ステージにいる(?:すべての)?『([^』]+)』のメンバーは/);
    if (stageAllSeriesM && !meta.grantToStageSeriesTag) {
      meta.grantToStageSeriesTag = stageAllSeriesM[1];
      meta.grantToStageSeriesMax = 99;
    }
    var otherOnlySeriesM = p.match(/(?:自分の)?ステージにいるほかの『([^』]+)』のメンバーは/);
    if (otherOnlySeriesM) {
      meta.grantToStageSeriesTag = otherOnlySeriesM[1];
      meta.grantToStageSeriesMax = 99;
      meta.grantExcludeSelf = true;
    }
    }
  ...
  if (
    (/ステージ(?:にいる)?(?:すべての)?メンバーは/.test(p) ||
      /ステージにいるすべてのメンバーは/.test(p) ||
      /ステージ(?:にいる)?のメンバーは/.test(p) ||
      /ステージのメンバーは/.test(p)) &&
    !/メンバー1人/.test(p) &&
    !/メンバー2人/.test(p) &&
    !/メンバー3人/.test(p)
  ...
  if (
    /控え室にあるメンバーカード1枚をデッキの一番上に置いてもよい/.test(p) &&
    /そうした場合/.test(p)
  ) {
    meta.optionalWaitingMemberToDeckTop = true;
    meta.optional = true;
    meta.hasOptionalCost = true;
  }
...
function parseAreaMemberHeartGrantMeta(p, segRaw) {
  ...
  var area = parseMemberLocationStageArea(s);
  if (!area) return null;
 ...
 */
/**
 * 「そのエネルギーカードは、次のターンのアクティブフェイズにアクティブしない」の有無。
 * @param {string} p
 * @returns {boolean}
 */
function textHasEnergySkipNextActivatePhase(p) {
  return /次のターンのアクティブフェイズにアクティブしない/.test(String(p || ""));
}

/**
 * 「自分の控え室から/にある『X』のカードをN枚(まで)デッキの(一番上|下)に置く(てもよい)」。
 * 『A』と『B』と『C』をそれぞれ1枚ずつ、という異なるタグ指定にも対応する。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyWaitingPickToDeckPatch(p) {
  var s = String(p || "");
  if (!/控え室(?:から|にある)/.test(s)) return null;
  var dest = null;
  if (/デッキの一番上に置/.test(s)) dest = "top";
  else if (/デッキの(?:一番)?下に置/.test(s)) dest = "bottom";
  if (!dest) return null;
  var tagsAll = (s.match(/『[^』]+』/g) || []).map(function (t) {
    return t.slice(1, -1);
  });
  var distinct = /それぞれ1枚ずつ/.test(s) && tagsAll.length >= 2 ? tagsAll : null;
  var cntM = s.match(/カードを?([０-９\d]+)枚/);
  var drawM = s.match(/カードを([０-９\d]+)枚引/);
  return {
    template: "waiting_pick_to_deck",
    waitingToDeckDest: dest,
    waitingToDeckCount: distinct ? distinct.length : cntM ? Number(normalizeFwDigits(cntM[1])) || 1 : 1,
    waitingToDeckUpTo: /枚まで/.test(s),
    waitingToDeckDistinctTags: distinct,
    deckDrawCount: drawM ? Number(normalizeFwDigits(drawM[1])) || 0 : 0,
    optional: /もよい/.test(s),
    filters: distinct ? {} : { seriesTag: tagsAll.length === 1 ? tagsAll[0] : null },
  };
}

/**
 * ライブ開始時「控え室からN枚選び→シャッフル→デッキ下→付与」（028 未来の音が聴こえる 等）。
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyLiveStartOptionalWaitingShuffleDeckBottomGrant(p, segRaw) {
  var s = String(p || "");
  if (!/控え室(?:から|にある)/.test(s)) return null;
  if (!/シャッフル/.test(s)) return null;
  if (!/デッキの(?:一番)?下に置/.test(s)) return null;
  if (!/もよい/.test(s)) return null;
  if (!/そうした/.test(s) && !/ライブ終了時まで/.test(s + segRaw)) return null;
  if (!/ライブ終了時まで/.test(s + segRaw)) return null;
  var cntM = s.match(/([０-９\d]+)枚選/);
  if (!cntM) return null;
  var blade = bladeGainFromIcons(segRaw, p);
  if (blade <= 0 && !/ブレード|icon_blade/i.test(String(segRaw || "") + s)) return null;
  var tagsAll = (s.match(/『[^』]+』/g) || []).map(function (t) {
    return t.slice(1, -1);
  });
  /** @type {Partial<ClassifiedAbility>} */
  var patch = {
    template: "live_start_optional_waiting_shuffle_deck_bottom_grant",
    optional: true,
    hasOptionalCost: true,
    waitingToDeckCount: Number(normalizeFwDigits(cntM[1])) || 1,
    waitingToDeckExact: true,
    waitingToDeckDest: "bottom",
    bladeGain: blade || 1,
    grantToAllStageMembers: true,
    requiresOnStage: true,
    filters: {
      pickType: T_MEMBER,
      seriesTag: tagsAll.length === 1 ? normalizeQuotedSeriesTag(tagsAll[0]) : null,
    },
  };
  return patch;
}
...
/**
 * スコア＋条件: エネルギー枚数差（相手よりN枚以上多い / 相手より多い）。
 * @param {Partial<ClassifiedAbility>} patch
 * @param {string} p
 */
function applyEnergyAdvantageScoreMeta(patch, p) {
  var advM = normalizeFwDigits(String(p || "")).match(/相手より(\d+)枚以上多い/);
  if (advM) {
    patch.minEnergyAdvantageOverOpponent = Number(advM[1]) || 1;
    return;
  }
  if (/エネルギーが相手より多い場合/.test(p) || /自分のエネルギーが相手より多い/.test(p)) {
    patch.requiresMoreEnergyThanOpponent = true;
  }
}

/**
 * スコア＋条件: エール公開の自分のカードがすべて指定シリーズ。
 * @param {Partial<ClassifiedAbility>} patch
 * @param {string} p
 */
function applyYellRevealedAllSeriesScoreMeta(patch, p) {
  var m = String(p || "").match(/公開された自分のカードがすべて『([^』]+)』/);
  if (m) patch.requiresYellRevealedAllSeriesTag = normalizeQuotedSeriesTag(m[1]);
}

/**
 * 「自分のデッキの一番(上|下)のカードを見る。それをデッキの(一番下|上からN番目)に置いてもよい」。
 * 1枚だけ覗いて別の位置へ差し込む効果。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckPeekRelocatePatch(p) {
  var s = String(p || "");
  var srcM = s.match(/デッキの一番(上|下)のカードを見る/);
  if (!srcM) return null;
  var dest = null;
  if (/デッキの一番下に置/.test(s)) dest = { deckRelocateTo: "bottom" };
  else {
    var nthM = s.match(/デッキの上から([０-９\d]+)番目に置/);
    if (nthM) dest = { deckRelocateTo: "nth_from_top", deckRelocateIndex: Number(normalizeFwDigits(nthM[1])) || 1 };
  }
  if (!dest) return null;
  return Object.assign(
    {
      template: "deck_peek_relocate",
      deckRelocateFrom: srcM[1] === "上" ? "top" : "bottom",
      optional: /もよい/.test(s),
    },
    dest,
  );
...
/**
 * 「自分のエネルギーデッキから、エネルギーカードN枚を（このメンバー／『X』のメンバー1人）の下に置く」。
 * エネルギー置き場を経由しない新規配置。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyEnergyDeckUnderMemberPatch(p) {
  var s = String(p || "");
  if (!/エネルギーデッキから/.test(s)) return null;
  var m = s.match(/エネルギーカード([０-９\d]+)?枚を(.*?)の下に置/);
  if (!m) return null;
  var target = m[2] || "";
  var isSelf = /このメンバー/.test(target);
  var seriesM = target.match(/『([^』]+)』/);
  return {
    template: "energy_deck_under_member",
    energyUnderCount: m[1] ? Number(normalizeFwDigits(m[1])) || 1 : 1,
    energyUnderTargetSelf: isSelf,
    optional: /もよい/.test(s),
    requiresOnStage: true,
    filters: isSelf ? {} : { pickType: T_MEMBER, seriesTag: seriesM ? normalizeQuotedSeriesTag(seriesM[1]) : null },
  };
}

/**
 * 「デッキの(上|下)からカードをN枚見る。好きな枚数を好きな順番でデッキの(上|下)に置き、残りを控え室に置く」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckLookReorderPatch(p) {
  var s = normalizeFwDigits(String(p || ""));
  var fromBottom = /デッキの下からカードを(\d+)枚見る/.test(s);
  var fromTop = /デッキの上からカードを(\d+)枚見る/.test(s);
  if (!fromBottom && !fromTop) return null;
  if (!/見る/.test(s) || !/好きな/.test(s)) return null;
  if (/手札に加/.test(s)) return null;
  var toBottom = /デッキの下に置/.test(s);
  var toTop = /デッキの上に置/.test(s);
  if (fromBottom && !toBottom) return null;
  if (fromTop && !toTop && !/残りを好きな順番でデッキの下に置/.test(s)) return null;
  var cntM = s.match(/デッキの(?:上|下)からカードを(\d+)枚見る/);
  /** @type {Partial<ClassifiedAbility>} */
  var lookReorderPatch = {
    template: "deck_top_look_reorder",
    deckTopCount: cntM ? Number(cntM[1]) || 3 : 3,
...
/**
 * 「控え室にあるメンバーカードN枚を、ステージにいるメンバー1人の下に置く」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyWaitingMemberUnderStagePatch(p) {
  var s = String(p || "");
  if (!/控え室に(?:ある|あり)/.test(s)) return null;
  if (!/メンバーカード([０-９\d]+)?枚/.test(s) && !/メンバーカード1枚/.test(s)) return null;
  if (!/ステージにいるメンバー1人の下に置/.test(s)) return null;
  if (/エネルギー/.test(s)) return null;
  var nM = s.match(/メンバーカード([０-９\d]+)枚/);
  return {
    template: "waiting_member_under_stage",
    waitingUnderCount: nM ? Number(normalizeFwDigits(nM[1])) || 1 : 1,
    optional: /もよい/.test(s),
    requiresOnStage: true,
...
/**
 * 起動: 山札ミル→控え室からメンバーを下に置き、元々ハートを下カードと同じにする。
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyKidouMillWaitingUnderCopyPrintedHeartsPatch(p, segRaw) {
  var s = normalizeFwDigits(String(p || ""));
  var millM = s.match(/デッキの上からカードを(\d+)枚控え室に置/);
  if (!millM) return null;
  if (!/控え室/.test(s) || !/下に置/.test(s)) return null;
  if (!/元々持つハート/.test(s + String(segRaw || ""))) return null;
  if (!/同じになる/.test(s + String(segRaw || ""))) return null;
  if (!/メンバーカード/.test(s)) return null;
  var costM = s.match(/コスト(\d+)以下/);
  var seriesM = s.match(/『([^』]+)』/);
  return {
    template: "kidou_mill_waiting_under_copy_printed_hearts",
    deckTopCount: Number(millM[1]) || 5,
    filters: {
      pickType: T_MEMBER,
      maxCost: costM ? Number(costM[1]) : undefined,
      seriesTag: seriesM ? normalizeQuotedSeriesTag(seriesM[1]) : undefined,
    },
  };
}

/**
 * 起動: 山札ミル→ミル内容が条件を満たすとき bullet から1つ選択。
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckMillConditionalPickOnePatch(p, segRaw) {
  var s = normalizeFwDigits(String(p || ""));
  var millM = s.match(/デッキの上からカードを(\d+)枚控え室/);
  if (!millM) return null;
  if (!/これにより控え室に置いた/.test(s)) return null;
  if (!/以下から1つを選ぶ/.test(s)) return null;
  if (!/ライブカード/.test(s) || !/ブレードハートを持たない/.test(s)) return null;
  var seriesM = s.match(/『([^』]+)』/);
  var choices = parseAbilityBulletChoices(segRaw);
  if (!choices.length) choices = parseAbilityBulletChoices(p);
  return {
    template: "deck_mill_conditional_pick_one",
    deckTopCount: Number(millM[1]) || 3,
    millRequireSeriesTag: seriesM ? normalizeQuotedSeriesTag(seriesM[1]) : null,
...
/**
 * 「ステージにいるメンバー1人をセンターエリアにポジションチェンジさせる」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyPickMemberToCenterPatch(p) {
  var s = String(p || "");
  if (!/ポジションチェンジ/.test(s)) return null;
  if (!/センターエリアに/.test(s)) return null;
  if (!/ステージにいるメンバー1人を/.test(s)) return null;
  return {
    template: "pick_stage_member_to_center",
    optional: /もよい/.test(s),
    requiresOnStage: true,
  };
}
...
/**
 * 「『X』のメンバーをN人までウェイトにしてもよい：1人につきこのカードのスコアを＋Mする」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyOptionalWaitMembersScorePerPatch(p) {
  var s = normalizeFwDigits(String(p || ""));
  var m = s.match(/『([^』]+)』のメンバーを(\d+)人までウェイトにしてもよ/);
  if (!m) return null;
  if (!/ウェイトにしたメンバー1人につき/.test(s) || !/スコアを＋(\d+)/.test(s)) return null;
  var scM = s.match(/スコアを＋(\d+)/);
  return {
    template: "live_start_optional_wait_members_score_per",
    waitMembersMax: Number(m[2]) || 3,
...
/**
 * 「このカードをライブカード置き場から手札に戻す。その後、手札をN枚控え室に置く」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyLiveReturnHandDiscardPatch(p) {
  var s = normalizeFwDigits(String(p || ""));
  if (!/ライブカード置き場から手札に戻す/.test(s)) return null;
  var dM = s.match(/手札を(\d+)枚控え室に置/);
  if (!dM) return null;
  return {
    template: "live_return_hand_then_discard",
    effectDiscardCount: Number(dM[1]) || 1,
  };
}

/**
 * 「デッキの一番下のカードを控え室に置いてもよい。それが「A」か「B」の場合、手札に加える」。
 * @param {string} p
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckBottomMillNamedHandPatch(p) {
  var s = String(p || "");
  if (!/デッキの一番下のカードを控え室に置いてもよ/.test(s)) return null;
  if (!/手札に加える/.test(s)) return null;
  /** @type {string[]} */
  var names = [];
  var re = /「([^」]+)」/g;
  var nm;
  while ((nm = re.exec(s)) !== null) names.push(nm[1]);
  if (!names.length) return null;
  return {
    template: "deck_bottom_optional_mill_named_hand",
    namedRecoverList: names,
    optional: true,
    hasOptionalCost: true,
  };
}
...
function enrichGrantJoujiPatch(p, segRaw, patch) {
  ...
  if (
    !patch.grantHeartSlotCount &&
    patch.requiredHeartSlot &&
    !patch.grantHeartSlotMap
  ) {
  ...
  if (!patch.grantHeartSlotMap) {
    var grantClauseMap = (String(segRaw || "") + String(p || "")).split(/ライブ終了時まで/)[1] || "";
    var multiHeartMap = parseNeedHeartMapFromSegmentRaw(grantClauseMap);
    var multiHeartKeys = Object.keys(multiHeartMap);
    if (multiHeartKeys.length > 1) {
      patch.grantHeartSlotMap = multiHeartMap;
    }
  }
  if (
  ...
  if (/下に置かれている名前の異なるメンバーカード1枚につき/.test(String(p || "") + String(segRaw || ""))) {
    patch.bladePerDistinctNameUnder = true;
    if (!patch.bladeGain) patch.bladeGain = bladeGainFromIcons(segRaw, p) || 1;
  }
  return Object.assign(patch, parseGrantJoujiMeta(p, segRaw));
}

/**
 * デッキミル→条件成立時ライブ終了時まで付与（bottom mill + シリーズ全員 / ライブ / 高コストメンバー等）
 * @param {string} p
 * @param {string} segRaw
 * @returns {Partial<ClassifiedAbility> | null}
 */
function classifyDeckMillConditionalGrantPatch(p, segRaw) {
  if (!/ライブ終了時まで/.test(p + segRaw)) return null;
  var grantBladeMill = bladeGainFromIcons(segRaw, p);
  var grantScoreMill =
    parseScorePlusFromText(p) || parseScorePlusFromText(String(segRaw || "").replace(/\{\{[^}]+\}\}/g, ""));

  /** @param {Partial<ClassifiedAbility>} patch */
  function finishMillGrantPatch(patch) {
    if (grantBladeMill > 0) patch.bladeGain = grantBladeMill;
    if (grantScoreMill > 0) patch.liveScoreGrant = grantScoreMill;
    return enrichGrantJoujiPatch(p, segRaw, patch);
  }

  if (/デッキの一番下のカードを控え室に置く/.test(p)) {
    var minCostM = p.match(/それがコスト(\d+)以上のメンバーカードの場合/);
    if (minCostM) {
      return finishMillGrantPatch({
        template: "deck_mill_conditional_grant",
        deckMillFrom: "bottom",
        deckTopCount: 1,
        millRequireMemberMinCost: Number(normalizeFwDigits(minCostM[1])) || 10,
      });
    }
  }

  var topMillM = p.match(/(?:自分の)?デッキの上からカードを(\d+)枚控え室に置/);
  if (topMillM) {
    var topDistinctBhM = p.match(/メンバーカードの中に([０-９\d]+)種類以上のブレードハートの色/);
    if (topDistinctBhM) {
      return finishMillGrantPatch({
        template: "deck_mill_conditional_grant",
        deckMillFrom: "top",
        deckTopCount: Number(normalizeFwDigits(topMillM[1])) || 3,
        millRequireDistinctBladeHeartColors: Number(normalizeFwDigits(topDistinctBhM[1])) || 2,
      });
    }
  }

  var botMillM = p.match(/デッキの下からカードを(\d+)枚控え室に置/);
  if (!botMillM) return null;
  var millN = Number(normalizeFwDigits(botMillM[1])) || 1;

  if (millN === 1 && /それがライブカードの場合/.test(p)) {
    return finishMillGrantPatch({
      template: "deck_mill_conditional_grant",
      deckMillFrom: "bottom",
      deckTopCount: 1,
      millRequireLive: true,
    });
  }
  ...
  if (seriesAllM) {
    ...
    var seriesPatch = {
      ...
      deckMillFrom: "bottom",
      deckTopCount: millN,
      millRequireAllSeriesMembers: normalizeQuotedSeriesTag(seriesAllM[1]),
    };
    if (/このメンバーをアクティブに/.test(p)) seriesPatch.millActivateSelf = true;
    return finishMillGrantPatch(seriesPatch);
  }
...
function classifyConditionalGrantJoujiInteractive(p, segRaw, trigger) {
  if (
    trigger === "live_start" &&
    /ステージにいる/.test(p) &&
    // 「ハートをすべて…にする」「ハートがすべて…になる」の両表記を拾う
    /元々持つハート[をがは]すべて/.test(p) &&
    /heart_0?\d/i.test(String(segRaw || "") + p) &&
    /ライブ終了時まで/.test(p)
  ) {
  ...
  if (
    trigger === "live_start" &&
    /手札の『([^』]+)』のカードを1枚公開してもよい/.test(p) &&
    /デッキの一番上か一番下に置/.test(p) &&
    /ライブ終了時まで/.test(p)
  ) {
    var handRevealSeriesM = p.match(/手札の『([^』]+)』のカード/);
    return {
      template: "live_start_hand_reveal_deck_place_blade",
      optional: true,
      hasOptionalCost: true,
      bladeGain: bladeGainFromIcons(segRaw, p) || 1,
      filters: { seriesTag: handRevealSeriesM ? handRevealSeriesM[1] : null },
      requiresOnStage: true,
    };
  }
  if (
...
function classifyDrawThenConditionalExtraDraw(p) {
  ...
  if (!/さらにカードを1枚引/.test(p)) return null;
  if (/相手もライブを成功している場合、さらにカードを1枚引/.test(p)) {
    var oppSuccDiscardM = p.match(/さらにカードを1枚引(?:き)?、手札を([０-９\d]+)枚控え室に置/);
    return {
      template: "draw_then_conditional_extra_draw",
      deckDrawCount: 1,
      extraDrawCount: 1,
      extraDrawCondType: "opponentLiveSuccessThisTurn",
      effectDiscardCount: oppSuccDiscardM ? Number(normalizeFwDigits(oppSuccDiscardM[1])) || 1 : 1,
    };
  }
...
/** @param {string} p @returns {number | null} */
function parseOppWaitExactPrintedBlade(p) {
  var s = normalizeFwDigits(String(p || ""));
  var exactM =
    s.match(/元々持つ[^。]*?ブレードの数がちょうど(\d+)つ/) ||
    s.match(/元々持つの数がちょうど(\d+)つ/) ||
    s.match(/ブレードの数がちょうど(\d+)つ/);
  return exactM ? Number(exactM[1]) : null;
}
...
function classifyOptionalSelfWaitEffect(p, base) {
  ...
  if (/相手のステージ/.test(p) && /ウェイト/.test(p)) {
    return Object.assign(patch, buildOppWaitStageMeta(p));
  }
  ...
  if (
    ...
    /公開/.test(p) &&
    /デッキの上|山札の上/.test(p) &&
    !/登場させるか、手札に加える/.test(p)
  ) {
    var owPickPatch = classifyDeckTopPickToHandPatch(p, "", {});
    if (owPickPatch) return Object.assign(patch, owPickPatch);
  }
  if (
    lookN != null &&
    /見る/.test(p) &&
  ...
  if (
    /手札.*ライブカードを1枚公開してもよい/.test(p) &&
    /成功ライブカード置き場にある/.test(p) &&
    /公開したカードを自分の成功ライブカード置き場に置く/.test(p)
  ) {
    return Object.assign(patch, {
  ...
  if (/成功ライブカード置き場にある/.test(p) && /手札に加/.test(p) && !/控え室から/.test(p)) {
    return Object.assign(patch, {
      template: "toujou_success_live_pick_hand",
      filters: parseAbilityPickFilters(p),
    });
  }
  var millDrawPartial = p.match(/デッキの上からカードを(\d+)枚控え室に置く/);
  if (millDrawPartial && /それらの中にライブカードがある場合/.test(p) && /カードを(\d+)枚引く/.test(p)) {
    var drawAfterPartial = p.match(/カードを(\d+)枚引く/);
    return Object.assign(patch, {
      template: "deck_mill_conditional_draw",
      deckTopCount: Number(millDrawPartial[1]) || 1,
      deckDrawCount: drawAfterPartial ? Number(drawAfterPartial[1]) || 1 : 1,
    });
  }
  var drawM = p.match(/カードを(\d+)枚引/);
  if (drawM && !/控え室から/.test(p) && !/山札の上からカードを/.test(p) && !/デッキの上からカードを\d+枚控え室/.test(p)) {
    return Object.assign(patch, {
  ...
  var td = parseDeckTopCount(p);
  var topPickPatch = classifyDeckTopPickToHandPatch(p, "", patch);
  if (topPickPatch) return Object.assign(patch, topPickPatch);
  var peekOptPatch = classifyDeckTopPeekOptionalWaitPatch(p, patch);
  if (peekOptPatch) return Object.assign(patch, peekOptPatch);
...
function classifyOptionalMemberWaitGrant(p, segRaw, base) {
  ...
  if (textHasHeartColorPickGrant(p)) {
    return enrichHeartColorPickGrantPatch(p, segRaw, {
...
/** wiki {{icon|表示}} を表示ラベルに残してから除去（・選択肢のブレード等を保持） */
function plainTextFromAbilityWikiMarkup(s) {
  return String(s || "")
    .replace(/\{\{[^|]+\|([^}]+)\}\}/g, "$1")
    .replace(/\{\{[^}]+\}\}/g, "");
}
...
export function parseAbilityBulletChoices(rawOrPlain) {
  ...
  lines.forEach(function (line) {
    var t = plainTextFromAbilityWikiMarkup(line).trim();
    if (!t) return;
    var m = t.match(/^[・•]\s*(.+)$/);
  ...
  if (!out.length) {
    ...
    while ((mm = re.exec(s)) !== null) {
      var bit = plainTextFromAbilityWikiMarkup(String(mm[1] || "")).trim();
      if (bit) out.push(bit);
    }
  }
  return out;
...
function bladeGainFromIcons(segRaw, p) {
  ...
  if (/ライブ終了時まで/.test(p) && /得る/.test(p)) {
    /**
     * 条件節の {{icon_blade}}（例: 「ブレードを4つ以上持つ」）を付与数に数えない。
     * 複数「ライブ終了時まで」がある場合は最初以降の付与節を結合して数える。
     */
    var grantParts = String(segRaw || "").split(/ライブ終了時まで/);
    var grantRaw = grantParts.length > 1 ? grantParts.slice(1).join("ライブ終了時まで") : "";
    var grantPlainParts = String(p || "").split(/ライブ終了時まで/);
    var grantPlain = grantPlainParts.length > 1 ? grantPlainParts.slice(1).join("ライブ終了時まで") : "";
    var fromGrantP = parseBladeGainCount(grantPlain);
    if (fromGrantP > 0) return fromGrantP;
    return countWikiBladeIcons(grantRaw);
  }
  return 0;
}
 ...
 * @param {ClassifiedAbility} base
 */
function classifyOptionalEnergyBladePerLiveFrame(segRaw, p, base) {
  if (!/ライブ中のカード1枚につき/.test(p)) return null;
  if (!/ライブ終了時まで/.test(p)) return null;
  var energyN = 0;
  var em = segRaw.match(/(?:icon_energy\.png\|E)+/);
  if (em) energyN = (em[0].match(/icon_energy\.png\|E/g) || []).length;
  if (energyN === 0) {
    var em2 = p.match(/(?:E|Ｅ){1,}/);
    if (em2) energyN = em2[0].length;
  }
  return Object.assign({}, base, {
    template: "live_start_optional_energy_blade_per_live_frame",
    bladeGainPerLiveFrame: bladeGainFromIcons(segRaw, p) || 1,
    optional: true,
    hasOptionalCost: true,
    costEnergy: true,
...
function classifyOptionalEnergyBladeUntilLiveEnd(segRaw, p, base) {
  if (/ほかの/.test(p) || /ほかの/.test(String(segRaw || ""))) return null;
  if (/このメンバー以外の/.test(p) || /このメンバー以外の/.test(String(segRaw || ""))) return null;
  if (/ライブ中のカード1枚につき/.test(p)) return null;
  if (!/ライブ終了時まで/.test(p)) return null;
  ...
  var maxPayM = p.match(/(\d+)つまで支払ってもよい/) || segRaw.match(/(\d+)つまで支払ってもよい/);
  if (maxPayM && /支払った.*につき/.test(p)) {
    var perBlade = countWikiBladeIcons(segRaw) || bladeGain || 1;
    return {
      template: "optional_energy_blade_until_live_end",
      optional: true,
      hasOptionalCost: true,
      costEnergy: true,
      costEnergyVariable: true,
      optionalEnergyMaxPay: Number(maxPayM[1]) || 2,
      bladeGainPerEnergy: perBlade,
      bladeGain: 0,
    };
  }
  return {
...
/** @param {string} p @returns {number | null} */
function matchHandTopReorderCount(p) {
  var m = String(p || "").match(/手札(?:を)?(\d+)枚(?:を)?好きな順番でデッキの上に置/);
  return m ? Number(m[1]) || null : null;
}

/** @param {AbilityPickFilters} f @param {string} segRaw @returns {AbilityPickFilters} */
function enrichPickFiltersFromSegRaw(f, segRaw) {
  if (f.minScore == null && /icon_score/i.test(String(segRaw || ""))) f.minScore = 1;
  if (f.minNeedHeartSlot == null) {
    var nhIconM = String(segRaw || "").match(/必要ハートに\{\{heart_0?(\d)[^}]*\}\}を([０-９\d]+)以上/);
    if (nhIconM) {
      f.minNeedHeartSlot = Number(nhIconM[1]);
      f.minNeedHeartValue = Number(normalizeFwDigits(nhIconM[2])) || 0;
    }
  }
  return f;
}
...
export function parseAbilityPickFilters(p, segRaw) {
  ...
  if (
    /控え室(?:から|にある)[^。；：]*?メンバーカード|手札のメンバーカード|メンバーカードを1枚手札|メンバーカードを1枚公開|のメンバーカードを1枚|メンバーカードを[^。；：]*?手札に加/.test(
      p,
    )
  ) {
    f.pickType = T_MEMBER;
  } else if (
    /控え室(?:から|にある)[^。；：]*?ライブカード|手札のライブカード|ライブカードを1枚手札|ライブカードを[^。；：]*?手札に加|『[^』]+』のライブカード|のライブカードを/.test(
      p,
    )
  ) {
    f.pickType = T_LIVE;
  ...
  if (f.minScore == null && /(スコアを持つ|icon_score)/.test(p)) {
    if (/ライブカード/.test(p) || f.pickType === T_LIVE) f.minScore = 1;
    else if (/成功ライブ/.test(p)) f.minScore = 1;
  }
  ...
  if (distinctYellM) {
    ...
    var yellSeriesMemberM = p.match(/エールにより公開[^。]*?『([^』]+)』のメンバーカードが([０-９\d]+)枚以上/);
    if (yellSeriesMemberM) {
      f.minYellRevealedSeriesMemberCount = Number(normalizeFwDigits(yellSeriesMemberM[2])) || 0;
      f.yellRevealedSeriesMemberTag = yellSeriesMemberM[1];
    }
  }
  var costRangeM = p.match(/コスト([０-９\d]+)以上([０-９\d]+)以下/);
  if (costRangeM) {
    f.minCost = Number(normalizeFwDigits(costRangeM[1]));
    f.maxCost = Number(normalizeFwDigits(costRangeM[2]));
  } else {
    var costAnyHandM = p.match(/手札のコストが([０-９\d]+)か([０-９\d]+)/);
    if (costAnyHandM) {
      f.costsAnyOf = [
        Number(normalizeFwDigits(costAnyHandM[1])),
        Number(normalizeFwDigits(costAnyHandM[2])),
      ];
    } else {
  ...
  if (/エールにより公開/.test(p) && /ブレードハートを持たない/.test(p) && /メンバーカード/.test(p)) {
    var yellNobhCountM = p.match(/ブレードハートを持たないメンバーカードが([０-９\d]+)枚以上/);
    if (yellNobhCountM) {
      f.minYellRevealedNoBladeHeartMembers = Number(normalizeFwDigits(yellNobhCountM[1])) || 1;
    } else {
  ...
  if (
    /エールにより公開/.test(p) &&
    /のうち、([０-９\d]+)種類以上/.test(p) &&
    /heart_0/i.test(String(segRaw || "") + p) &&
    !/メンバーカードがそれぞれ/.test(p) &&
    !/ブレードハートの中に/.test(p)
  ) {
    var yellHeartKindsM = p.match(/([０-９\d]+)種類以上/);
    f.minYellRevealedHeartColorKinds = yellHeartKindsM
      ? Number(normalizeFwDigits(yellHeartKindsM[1])) || 3
      : 3;
  }
  if (
    /ブレードハートを持たない/.test(p) &&
  ...
  if (
    /ブレードハートを持たない/.test(p) &&
    !f.requiresYellRevealedNoBladeHeartMember &&
    !f.pickFilterAlternatives &&
    /(?:その中から|見た|デッキの上|山札の上)/.test(p) &&
    /手札に加/.test(p)
  ) {
  ...
  if (needM) {
    ...
    var heartPlainM = String(color || "").match(/heart_?0?(\d)/i);
    if (f.minNeedHeartSlot == null && heartPlainM) {
      f.minNeedHeartSlot = Number(heartPlainM[1]);
    }
  }
  ...
  var otherLiveFrameM = p.match(/ライブカード置き場に「([^」]+)」以外の『([^』]+)』のライブカードがある/);
  if (otherLiveFrameM) {
    f.requiresOtherSeriesLiveOnFrameExcludeName = otherLiveFrameM[1];
    f.requiresOtherSeriesLiveOnFrameTag = otherLiveFrameM[2];
  }
  if (/デッキがリフレッシュしていた/.test(p)) f.requiresDeckRefreshedThisTurn = true;
  var charStage = p.match(/ステージに「([^」]+)」がいる/);
  if (charStage) f.characterNameOnStage = charStage[1];
  var namedOrMinM = p.match(/ステージに(.+?)のうち、名前の異なるメンバーが([０-９\d]+)人以上/);
  if (namedOrMinM) {
    /** @type {string[]} */
    var namedList = [];
    var namedListRe = /「([^」]+)」/g;
    var namedListM;
    while ((namedListM = namedListRe.exec(namedOrMinM[1])) !== null) namedList.push(namedListM[1]);
    if (namedList.length) {
      f.minDistinctNamedStageMembersFromList = Number(normalizeFwDigits(namedOrMinM[2])) || 0;
      f.namedStageMemberList = namedList;
    }
  }
  var stageNamedAreas = parseStageNamedMemberAreaRequirements(p);
  if (stageNamedAreas) f.requiresStageNamedMemberAreas = stageNamedAreas;
  ...
  if (/このゲームの1ターン目のライブフェイズ/.test(p)) f.requiresFirstGameLivePhase = true;
  if (/アクティブ状態の自分のエネルギーがある/.test(p)) f.requiresAnyActiveEnergy = true;
  if (/このメンバーがエリアを移動している/.test(p)) {
    f.requiresSelfMovedThisTurn = true;
  } else if (/このターン中にエリアを移動/.test(p) && !/または|か、/.test(p)) {
  ...
  var centerBladeM =
    p.match(/センターエリアに.*ブレード.*を([０-９\d]+)つ以上持つ/) ||
    p.match(/センターエリアにを([０-９\d]+)つ以上持つ『/);
  if (centerBladeM) {
    f.stageArea = "center";
    f.minStageAreaMemberBlade = Number(normalizeFwDigits(centerBladeM[1])) || 0;
  }
  var stageBladeSumM = p.match(
    /ステージにいるメンバーが持つ[^。]*?(?:ブレード|icon_blade)[^。]*?合計が([０-９\d]+)以上/,
  );
  if (!stageBladeSumM) {
    stageBladeSumM = p.match(/メンバーが持つ[^。]*?合計が([０-９\d]+)以上/);
  }
  if (stageBladeSumM) {
    f.minStageMemberBladeSum = Number(normalizeFwDigits(stageBladeSumM[1])) || 0;
  }
  var absentBladeM =
    p.match(/ブレード.*を([０-９\d]+)つ以上持つ『([^』]+)』のメンバーがいない場合/) ||
    p.match(/を([０-９\d]+)つ以上持つ『([^』]+)』のメンバーがいない場合/);
  if (absentBladeM) {
    f.forbidStageMemberMinBlade = Number(normalizeFwDigits(absentBladeM[1])) || 0;
    f.forbidStageMemberMinBladeSeries = absentBladeM[2];
  }
  if (
    /ライブ開始時.*能力も.*ライブ成功時.*能力も持たない/.test(p) ||
    /ライブ中のライブカードに、.*能力も.*能力も持たない/.test(p)
  ) {
    f.requiresLiveFrameNoStartSuccessAbility = true;
  }
  ...
  if (!/成功ライブカード置き場/.test(p)) {
    ...
    var liveMidM = p.match(/ライブ中のカードが([０-９\d]+)枚以上/);
  ...
  var distinctSeriesStageM = p.match(
    /(?:自分の)?ステージに名前の異なる『([^』]+)』のメンバーが([０-９\d]+)人(?:以上|いる)/,
  );
  if (distinctSeriesStageM) {
    ...
    var stageSeriesM = p.match(
      /(?:自分の)?ステージに『([^』]+)』のメンバーが([０-９\d]+)人(?:以上|いる)/,
    );
    if (stageSeriesM) {
  ...
  var memOrLiveM = p.match(
    /コスト([０-９\d]+)以下のメンバーカードか、スコア([０-９\d]+)以下のライブカード/,
  );
  if (memOrLiveM) {
    f.pickFilterAlternatives = [
      { pickType: T_MEMBER, maxCost: Number(normalizeFwDigits(memOrLiveM[1])) },
      { pickType: T_LIVE, pickMaxScore: Number(normalizeFwDigits(memOrLiveM[2])) },
    ];
    f.pickType = null;
    f.maxCost = null;
  }
  ...
  var waitAnyM = p.match(/控え室にカードが([０-９\d]+)枚以上/);
  if (waitAnyM && !/『/.test(waitAnyM[0])) {
    f.minWaitingAnyCardCount = Number(normalizeFwDigits(waitAnyM[1])) || 0;
  }
  ...
  var distinctStageM = p.match(/自分のステージに名前の異なるメンバーが([０-９\d]+)人以上/);
  if (distinctStageM) f.minDistinctStageMemberNames = Number(normalizeFwDigits(distinctStageM[1])) || 0;
  if (/余剰ハートを持たない/.test(p) && !/余剰ハートを2つ以上/.test(p)) {
    f.requiresZeroSurplusHearts = true;
  }
  /* 「『A』のメンバーカードかブレードハートを持つ『B』のメンバーカード」の OR 選択 */
  var altBhPickM = p.match(/『([^』]+)』のメンバーカードかブレードハートを持つ『([^』]+)』のメンバーカード/);
  if (altBhPickM) {
    f.pickFilterAlternatives = [
      { pickType: T_MEMBER, seriesTag: normalizeQuotedSeriesTag(altBhPickM[1]) },
      {
        pickType: T_MEMBER,
        seriesTag: normalizeQuotedSeriesTag(altBhPickM[2]),
        requiresHasBladeHeart: true,
      },
    ];
    f.seriesTag = null;
  }
  if (/このターン、自分のステージにいるほかのメンバーがエリアを移動している場合/.test(p)) {
    f.requiresOtherStageMemberMovedThisTurn = true;
  }
  if (
    f.minStageSeriesMembers != null &&
    f.minStageSeriesMembersTag &&
    f.seriesTag === f.minStageSeriesMembersTag &&
    !/控え室から『/.test(p)
  ) {
    f.seriesTag = null;
  }
...
/** ライブカードの special_heart ドロー説明（括弧内）。効果ではなく分類から除外する。 */
function stripLiveDrawYellReminderParenthetical(p) {
  return String(p || "").replace(
    /\n?\s*\(エールをすべて行った後、エールで出た[\s\S]*?カードを1枚引く。\)/g,
    "",
  );
}
...
function isCompoundLiveScoreEffectText(p) {
  var effect = scoreEffectPartAfterCondition(p);
  if (/のうち1色につき、このカードのスコア/.test(effect)) return false;
...
/**
 * 「ライブ終了時まで」＋ハート／ブレード付与だが {{jyouji}} マーカーがない単純付与。
 * 常時セッション合成の対象外（playBonus*Always へ直接付与する）。
 * @param {string} segRaw
 */
export function isPlainLiveEndHeartBladeGrantSegment(segRaw) {
  var s = String(segRaw || "");
  if (!/ライブ終了時まで/.test(s)) return false;
  if (/\{\{jyouji/i.test(s) || /「\{\{jyouji/.test(s)) return false;
  if (
    /合計スコア|このカードのスコア|ステージのコスト|元々持つハート|ライブできない|エールによって公開|枚数が[０-９\d]+枚減|必要ハートに含まれる|heart_00|heart_07|icon_all/i.test(
      s,
    )
  ) {
    return false;
  }
  if (!/を得る/.test(s)) return false;
  if (/\{\{heart_|heart_0[1-6]/i.test(s)) return true;
  if (/\{\{[^}]*blade/i.test(s) || /ブレード/.test(s)) return true;
  return false;
}
...
export function catalogCardMatchesPickFilters(cat, filters) {
  ...
  if (filters.pickFilterAlternatives && filters.pickFilterAlternatives.length) {
    for (var ai = 0; ai < filters.pickFilterAlternatives.length; ai++) {
      if (catalogCardMatchesPickFilters(cat, filters.pickFilterAlternatives[ai])) return true;
    }
    return false;
  }
  ...
  if (filters.costsAnyOf && filters.costsAnyOf.length) {
    var costAny = Number(cat.cost != null ? cat.cost : cat.score);
    if (!Number.isFinite(costAny)) return false;
    var costHit = false;
    for (var cai = 0; cai < filters.costsAnyOf.length; cai++) {
      if (costAny === Number(filters.costsAnyOf[cai])) {
        costHit = true;
        break;
      }
    }
    if (!costHit) return false;
  }
  ...
  if (filters.requiresHasBladeHeart && !catalogCardHasBladeHeart(cat)) return false;
  if (filters.requiresNoBladeHeart && catalogCardHasBladeHeart(cat)) return false;
  return true;
}

/** カードがブレードハート（blade_heart のいずれか）を持つか */
export function catalogCardHasBladeHeart(cat) {
  var blh = cat && cat.blade_heart;
  if (!blh || typeof blh !== "object") return false;
  for (var k in blh) {
    if (!Object.prototype.hasOwnProperty.call(blh, k)) continue;
    if (Number(blh[k]) > 0) return true;
  }
  return false;
}
...
function _classifyCardAbilityCore(card, trigger, segmentRawOverride) {
  ...
  // 「エネルギー置き場にあるエネルギーN枚をエネルギーデッキに置く（もよい）：」は
  // E支払い（総合ルール 5.9.1 のウェイト化）ではなく、置き場からエネルギーデッキへ戻すコスト。
  var energyToDeckCostM = costPart.match(
    /エネルギー(?:置き場にある)?(?:エネルギー)?([０-９\d]+)?枚(?:を)?エネルギーデッキに置/,
  );
  if (energyToDeckCostM) {
    base.costEnergyToDeck = true;
    base.costEnergyToDeckCount = energyToDeckCostM[1]
      ? Number(normalizeFwDigits(energyToDeckCostM[1])) || 1
      : 1;
  }
  // 「エネルギー置き場にあるエネルギーN枚をこのメンバーの下に置く：」
  var energyUnderCostM = costPart.match(
    /エネルギー置き場にあるエネルギー([０-９\d]+)?枚をこのメンバーの下に置/,
  );
  if (energyUnderCostM) {
    base.costEnergyUnderSelf = true;
    base.costEnergyUnderSelfCount = energyUnderCostM[1]
      ? Number(normalizeFwDigits(energyUnderCostM[1])) || 1
      : 1;
  }
  base.costOrAlt =
    /ウェイトにするか/.test(costPart) ||
    (/手札.*控え室/.test(costPart) && /か/.test(costPart) && base.costSelfWait);
  base.handDiscardToWaiting = parseHandDiscardCostFromCostPart(costPart);
  if (/手札をすべて控え室/.test(costPart)) {
    base.costHandDiscardAll = true;
  }
  if (/手札のライブカードを1枚控え室/.test(costPart)) {
    base.handDiscardToWaiting = 1;
    base.costHandDiscardPickType = T_LIVE;
  }
  if (
    base.costSelfWait &&
    base.handDiscardToWaiting &&
    /メンバーをウェイトにし/.test(costPart) &&
    /手札.*置いてもよい/.test(costPart) &&
    !base.costOrAlt
  ) {
    base.costHandDiscardOptional = true;
    base.hasOptionalCost = false;
    /* 「ウェイトにし、手札…置いてもよい」はコスト内の任意手札捨て。能力全体を任意にしない（ウェイトは必須） */
    base.optional = false;
  }
  ...
  if (enterKidou) {
    ...
    if (
      (/手札の「/.test(p) || /手札のコストが/.test(p)) &&
      /公開/.test(p) &&
      /下に置/.test(p) &&
      !/控え室に置/.test(p.split("：")[1] || p)
    ) {
      var drawRevealUnderKd = p.match(/カードを(\d+)枚引/);
      return kidouT({
        template: "kidou_hand_reveal_to_under",
        filters: Object.assign(parseAbilityPickFilters(p), {
          pickType: T_MEMBER,
        }),
        deckDrawCount: drawRevealUnderKd ? Number(normalizeFwDigits(drawRevealUnderKd[1])) : 0,
        perTurnLimit: perTurn,
      });
    }
    ...
    var deckMillPcM = p.match(/デッキの上からカードを(\d+)枚控え室に置く：/);
    if (deckMillPcM && /ポジションチェンジする/.test(p)) {
      return kidouT(
        Object.assign(
          {
            template: "kidou_deck_top_wait_position_change",
            deckTopCount: Number(deckMillPcM[1]) || 3,
            perTurnLimit: perTurn,
          },
    ...
    if (
      /好きなハートの色を1つ指定/.test(p) &&
      /デッキの上からカードを(\d+)枚公開/.test(p) &&
      /指定した色のハートを持つメンバーカードと必要ハートに指定した色を含むライブカードが合計([０-９\d]+)枚含まれる/.test(p)
    ) {
      var deckRevealN = p.match(/デッキの上からカードを(\d+)枚公開/);
      var deckMatchN = p.match(/合計([０-９\d]+)枚含まれる/);
      return kidouT({
        template: "kidou_heart_color_deck_reveal_pick",
        handDiscardToWaiting: /手札を1枚控え室に置/.test(p) ? 1 : null,
        deckTopCount: deckRevealN ? Number(deckRevealN[1]) : 5,
        deckRevealMatchCount: deckMatchN ? Number(normalizeFwDigits(deckMatchN[1])) : 5,
    ...
    var millUnderCopyKd = classifyKidouMillWaitingUnderCopyPrintedHeartsPatch(p, segRaw);
    if (millUnderCopyKd) return kidouT(millUnderCopyKd);

    var millPickOneKd = classifyDeckMillConditionalPickOnePatch(p, segRaw);
    if (millPickOneKd) return kidouT(millPickOneKd);
    ...
    if (/ライブ終了時まで/.test(p + segRaw)) {
      var millGrantKd = classifyDeckMillConditionalGrantPatch(p, segRaw);
      if (millGrantKd) return kidouT(millGrantKd);
    ...
    if (/ステージから控え室/.test(p) && /手札に加/.test(p)) {
      ...
      var kidouStgWaitEffectPart = p.indexOf("：") >= 0 ? p.split("：").slice(1).join("：") : p;
      /** @type {Partial<ClassifiedAbility>} */
      var stgWaitPatch = {
        template: "kidou_stage_wait_pick_hand",
        filters: parseAbilityPickFilters(kidouStgWaitEffectPart),
        pickAny: /控え室からカードを1枚/.test(kidouStgWaitEffectPart),
        energyActiveCount: actEnStgWait ? Number(actEnStgWait[1]) || 1 : undefined,
      };
      var enToDeckKidouM = kidouStgWaitEffectPart.match(
        /エネルギー(?:置き場にある)?(?:エネルギー)?([０-９\d]+)?枚(?:を)?エネルギーデッキに置/,
      );
      if (enToDeckKidouM) {
        stgWaitPatch.energyToDeckCount = enToDeckKidouM[1]
          ? Number(normalizeFwDigits(enToDeckKidouM[1])) || 1
          : 1;
      }
    ...
    if (/手札のライブカードを1枚公開する：/.test(p) && /カード名がすべて含まれるライブカード/.test(p)) {
      return kidouT({
        template: "kidou_reveal_live_wait_pick_name_contains",
        filters: { pickType: T_LIVE },
        perTurnLimit: perTurn,
      });
    }
    ...
    if (edWaitKd) {
      return kidouT({
        ...
        energySkipNextActivate: textHasEnergySkipNextActivatePhase(p),
      });
    }
    ...
    if (
      ...
      var pickPlayerDrawKd = p.match(/カードを(\d+)枚引/);
      return kidouT({
        template: "live_start_pick_player_waiting_deck_bottom",
        deckBottomPickMax: 1,
        deckDrawOnSuccess: pickPlayerDrawKd ? Number(pickPlayerDrawKd[1]) || 1 : 1,
        filters: { pickType: T_LIVE },
        perTurnLimit: perTurn,
      });
    }
    ...
    if (
      ...
      return kidouT({
        ...
        deckTopCount: parseDeckTopCount(p) || 5,
        deckTopPickMax: 1,
        filters: Object.assign(parseAbilityPickFilters(p), { pickType: T_LIVE }),
        optional: true,
        perTurnLimit: perTurn,
    ...
    var topPickKd = classifyDeckTopPickToHandPatch(p, segRaw);
    if (topPickKd) return kidouT(topPickKd);
    ...
    // 「エネルギー置き場…下に置く：相手…ブレード数が下にあるエネ枚数+1以下をウェイト」
    if (
      base.costEnergyUnderSelf &&
      /相手のステージにいる/.test(p) &&
      /ウェイトにする/.test(p) &&
      /下にあるエネルギーカードの枚数に1を足した数以下/.test(p)
    ) {
      return kidouT({
        template: "kidou_energy_under_opp_wait_by_under",
        costEnergyUnderSelf: true,
        costEnergyUnderSelfCount: base.costEnergyUnderSelfCount || 1,
        perTurnLimit: perTurn,
        requiresOnStage: true,
      });
    }
    // 「エネルギー置き場…下に置く：控え室からコストN以下の『X』を空きエリアにウェイト登場」
    if (
      base.costEnergyUnderSelf &&
      /控え室から/.test(p) &&
      /登場させる/.test(p) &&
      ...
      var keUnder = p.match(/コスト([０-９\d]+)以下/);
      var serUnder = p.match(/『([^』]+)』/);
      return kidouT({
        template: "kidou_energy_under_waiting_enter",
        costEnergyUnderSelf: true,
        costEnergyUnderSelfCount: base.costEnergyUnderSelfCount || 1,
        perTurnLimit: perTurn,
        requiresOnStage: true,
        filters: {
          pickType: T_MEMBER,
          maxCost: keUnder ? Number(normalizeFwDigits(keUnder[1])) || 2 : 2,
          seriesTag: serUnder ? normalizeQuotedSeriesTag(serUnder[1]) : null,
        },
        enterAsWait: true,
        blockEntryThisTurn: /このターン[^。]*登場できない/.test(p),
      });
    }
    // 「手札をN枚控え室に置く：このメンバーとほかの『X』1人を選び、登場能力をそれぞれ発動」
    if (
      ...
      var hdTwin = p.match(/手札を([０-９\d]+)枚控え室に置/);
      var serTwin = p.match(/ほかの『([^』]+)』/);
      return kidouT({
        template: "kidou_self_and_other_resolve_toujou",
        handDiscardToWaiting: hdTwin ? Number(normalizeFwDigits(hdTwin[1])) || 2 : 2,
        perTurnLimit: perTurn,
        requiresOnStage: true,
        requiresCenter: /センター/.test(segRaw) || parseStageAreaConstraint(segRaw) === "center",
        filters: { seriesTag: serTwin ? normalizeQuotedSeriesTag(serTwin[1]) : null, pickType: T_MEMBER },
      });
    }
  ...
  if (enterToujyou) {
    ...
    function twT(obj) {
      ...
      return withTrigger("toujyou", patch);
    }

    if (!segmentRawOverride && trigger === "toujyou") {
      var toujyouSegsOnly = splitAbilityByTriggers(cardAbilityRawText(card)).filter(function (s) {
        return s.trigger === "toujyou";
      });
      if (toujyouSegsOnly.length > 1) {
        return twT({
          template: "ability_sequence",
          steps: toujyouSegsOnly.map(function (s) {
            return classifyCardAbility(card, "toujyou", s.text, { skipCompose: true });
          }),
          requiresOnStage: true,
        });
      }
    ...
    if (/ステージにいるメンバー.*それぞれ好きなエリアに移動/.test(p) || /フォーメーションチェンジしてもよい/.test(p)) {
      ...
      var stageOnlyAnyM = p.match(/ステージに(?:いる)?『([^』]+)』か『([^』]+)』のメンバーのみ/);
      if (stageOnlyAnyM) {
        relocatePatch.requiresStageOnlySeriesAny = [stageOnlyAnyM[1], stageOnlyAnyM[2]];
      }
      if (/『SaintSnow』のメンバーが移動した場合/.test(p) && /ライブ終了時まで/.test(p + segRaw)) {
        relocatePatch.bladeGain = bladeGainFromIcons(segRaw, p) || 2;
      }
      return twT(relocatePatch);
    }
    ...
    if (
      ...
      /それらがすべて/.test(p) &&
      /メンバー/.test(p) &&
      /heart_04|heart04|h04/i.test(String(segRaw || "") + p)
    ) {
      return twT({
    ...
    if (
      ...
      /すべて/.test(p) &&
      /メンバー/.test(p) &&
      /heart_05|heart05|h05/i.test(String(segRaw || "") + p)
    ) {
      return twT({
        template: "toujou_deck_top_wait_if_all_heart",
        deckTopCount: deckTopMem ? Number(deckTopMem[1]) : 3,
    ...
    if (
      /ステージに「[^」]+」か「[^」]+」か「[^」]+」がいる場合/.test(p) &&
      /エネルギーを1枚アクティブ/.test(p) &&
      /控え室から/.test(p) &&
      /手札に加/.test(p)
    ) {
      var charAnyTjM = p.match(/「([^」]+)」か「([^」]+)」か「([^」]+)」がいる/);
      return twT({
        template: "toujou_named_stage_activate_recover_wait",
        energyActiveCount: 1,
        requiresOnStage: true,
        filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), {
          namedStageMemberList: charAnyTjM ? [charAnyTjM[1], charAnyTjM[2], charAnyTjM[3]] : [],
          minNamedStageMemberFromList: 1,
        }),
      });
    }
    if (
    ...
    var payWaitRecoverGrantM =
      base.costEnergy && base.hasOptionalCost
        ? p.match(/：自分の控え室から『([^』]+)』のカードを(\d+)枚手札に加える。そうした場合/)
        : null;
    if (payWaitRecoverGrantM && /ライブ終了時まで/.test(p + segRaw) && bladeGainFromIcons(segRaw, p) > 0) {
      return twT({
        template: "toujou_optional_pay_wait_recover_grant",
        optional: true,
        requiresOnStage: true,
        bladeGain: bladeGainFromIcons(segRaw, p),
        recoverCount: Number(normalizeFwDigits(payWaitRecoverGrantM[2])) || 1,
        recoverFilters: { seriesTag: normalizeQuotedSeriesTag(payWaitRecoverGrantM[1]) },
      });
    }
    if (/ライブ終了時まで/.test(p + segRaw) && /を得る/.test(p)) {
      var millGrantTj = classifyDeckMillConditionalGrantPatch(p, segRaw);
      if (millGrantTj) {
        return twT(Object.assign({ requiresOnStage: true }, millGrantTj));
      }
      ...
      // ハートだけを付与する登場能力もここで拾う（スロットは enrichGrantJoujiPatch が抽出する）
      var grantHeartTj = /ライブ終了時まで[^。]*?\{\{(?:heart_0?\d|icon_all)/i.test(String(segRaw || "") + String(p || ""));
      if (/常時/.test(segRaw) || grantScoreTj > 0 || grantBladeTj > 0 || grantHeartTj) {
        return twT(
          enrichGrantJoujiPatch(p, segRaw, {
            template: "grant_jouji_session",
            liveScoreGrant: grantScoreTj || (grantBladeTj > 0 || grantHeartTj ? 0 : 1),
    ...
    if (
      ...
      /デッキの(?:一番)?下/.test(p) &&
      /枚まで/.test(p) &&
      /デッキに置いたカード1枚につき/.test(p + segRaw) &&
      /エネルギー.*アクティブ/.test(p + segRaw)
    ) {
      var wtdActTjEarly = classifyWaitingPickToDeckPatch(p);
      if (wtdActTjEarly) {
        var actEnPerTjEarlyM = (p + segRaw).match(/エネルギーを([０-９\d]+)枚アクティブ/);
        return twT(
          Object.assign({}, wtdActTjEarly, {
            template: "waiting_to_deck_bottom_activate_per",
            energyActivePerMoved: actEnPerTjEarlyM ? Number(normalizeFwDigits(actEnPerTjEarlyM[1])) || 1 : 1,
            filters: mergeAbilityPickFilters(parseAbilityPickFilters(p, segRaw), {
              requiresNoBladeHeart: /ブレードハートを持たない/.test(p),
            }),
            requiresOnStage: true,
          }),
        );
    ...
    var edUnderTj = classifyEnergyDeckUnderMemberPatch(p);
    if (edUnderTj) return twT(edUnderTj);
    var peekTj = classifyDeckPeekRelocatePatch(p);
    if (peekTj) return twT(peekTj);
    ...
    if (handStage) {
      ...
      if (/既にメンバーがいるエリア/.test(p)) handStagePatch.allowOccupiedStageColumn = true;
      if (
        /このメンバーよりコストが低いメンバーからバトンタッチ/.test(p) &&
        !/このメンバーよりコストが低い『/.test(p)
      ) {
        handStagePatch.requiresBatonFromLowerCostMember = true;
      }
      return twT(handStagePatch);
    }
    var energyPerDrawM = p.match(/自分のエネルギー(\d+)枚につき.*カードを(\d+)枚引/);
    if (energyPerDrawM) {
      return twT({
        template: "toujou_draw_per_energy_unit",
        energyPerDrawUnit: Number(energyPerDrawM[1]) || 6,
        deckDrawCount: Number(energyPerDrawM[2]) || 1,
        requiresOnStage: true,
      });
    }
    ...
    if (
      /バトンタッチして登場した場合/.test(p) &&
      /自分と相手はそれぞれ/.test(p) &&
      /手札のカードを(\d+)枚まで選び/.test(p) &&
      /選んだカード以外/.test(p) &&
      /シャッフル/.test(p) &&
      /デッキの下/.test(p) &&
      /カードを(\d+)枚引/.test(p)
    ) {
      var batonSeriesKeepM = p.match(/『([^』]+)』のメンバーからバトンタッチ/);
      var handKeepM = p.match(/手札のカードを(\d+)枚まで選び/);
      var batonKeepDrawM = p.match(/カードを(\d+)枚引/);
      return twT({
        template: "toujou_baton_both_keep_hand_shuffle_deck_bottom_draw",
        requiresBatonFromSeriesTag: batonSeriesKeepM ? batonSeriesKeepM[1] : null,
        handKeepMax: Number(handKeepM && handKeepM[1]) || 3,
        deckDrawCount: Number(batonKeepDrawM && batonKeepDrawM[1]) || 3,
        requiresOnStage: true,
      });
    }
    if (
    ...
    var lookReorderPatchTj = classifyDeckLookReorderPatch(p);
    if (
      lookReorderPatchTj &&
      lookReorderPatchTj.template === "deck_top_look_reorder" &&
      /デッキの上に置/.test(p) &&
      !/手札に加/.test(p)
    ) {
      return twT(Object.assign({ requiresOnStage: true }, lookReorderPatchTj));
    }
    ...
    if (topPick != null && /登場させるか、手札に加える/.test(p)) {
      return twT({
        template: "deck_top_pick_enter_or_hand",
        deckTopCount: topPick,
        requiresOnStage: true,
        filters: parseAbilityPickFilters(p),
      });
    }
    var topPickRecoverEarly = classifyDeckTopPickToHandPatch(p, segRaw, { requiresOnStage: true });
    if (topPickRecoverEarly) return twT(enrichOptionalHandDiscardCost(base, topPickRecoverEarly, p));
    var millDrawTj = p.match(/デッキの上からカードを(\d+)枚控え室に置く/);
    if (millDrawTj && /それらの中にライブカードがある場合/.test(p) && /カードを(\d+)枚引く/.test(p)) {
      var drawAfterTj = p.match(/カードを(\d+)枚引く/);
      return twT({
        template: "deck_mill_conditional_draw",
        deckTopCount: Number(millDrawTj[1]) || 1,
        deckDrawCount: drawAfterTj ? Number(drawAfterTj[1]) || 1 : 1,
        requiresOnStage: true,
      });
    }
    if (/能力を持たないメンバーからバトンタッチして登場した場合/.test(p) && /カードを.*枚引/.test(p)) {
      var batonNoAbDrawM = p.match(/カードを(\d+)枚引/);
      return twT({
        template: "draw_from_deck",
        deckDrawCount: batonNoAbDrawM ? Number(batonNoAbDrawM[1]) || 1 : 1,
        requiresBatonFromNoAbilityMember: true,
        requiresOnStage: true,
      });
    }
    var drawM = p.match(/カードを(\d+)枚引/);
    ...
    if (
      ...
      return twT({
        template: "live_start_pick_player_waiting_deck_bottom",
        deckBottomPickMax: 1,
        deckDrawOnSuccess: Number(drawM[1]) || 1,
        requiresOnStage: true,
        filters: { pickType: T_LIVE },
      });
    }
    // メンバーカード版（ライブ開始時と同型。「デッキの下」「デッキの一番下」の両表記を許容）
    if (/自分か相手を選ぶ/.test(p) && /控え室.*メンバーカード/.test(p) && /デッキの(?:一番)?下/.test(p)) {
      return twT({
        template: "live_start_pick_player_waiting_deck_bottom",
    ...
    if (
      ...
      if (/ライブ終了時まで.*ライブできない/.test(p)) {
        drawDeckPatch.grantPlayerCannotLiveUntilLiveEnd = true;
      }
      return twT(enrichOptionalHandDiscardCost(base, drawDeckPatch, p));
    }
    if (
      /手札.*ライブカードを1枚公開してもよい/.test(p) &&
      /成功ライブカード置き場にある/.test(p) &&
    ...
    if (/控え室(?:から|にある)/.test(p) && /手札に加/.test(p)) {
      if (
        /ライブ開始時.*能力/.test(p + segRaw) &&
        /無効にしてもよい/.test(p) &&
        /これにより無効にした場合/.test(p)
      ) {
        var recoverPartDisableLs = p.split(/無効にした場合[,，]?/)[1] || p;
        var disableSeriesTjM = p.match(/ステージにいる『([^』]+)』のメンバー/);
        return twT({
          template: "toujou_optional_disable_live_start_recover_wait",
          optional: true,
          requiresOnStage: true,
          disableTargetSeriesTag: disableSeriesTjM ? disableSeriesTjM[1] : null,
          filters: parseAbilityPickFilters(recoverPartDisableLs),
        });
      }
      /** @type {Partial<ClassifiedAbility>} */
      var waitPickPatch = {
      ...
      var handSeriesCostM = p.match(/手札の『([^』]+)』のカードを(\d+)枚控え室に置いてもよい/);
      if (handSeriesCostM) {
        waitPickPatch.optional = true;
        waitPickPatch.hasOptionalCost = true;
        waitPickPatch.handDiscardToWaiting = Number(normalizeFwDigits(handSeriesCostM[2])) || 1;
        waitPickPatch.handDiscardSeriesTag = handSeriesCostM[1];
      }
      var handNoBhCostM = p.match(
        /手札のブレードハートを持たないメンバーカードを(\d+)枚まで控え室に置いてもよい/,
      );
      if (handNoBhCostM) {
        waitPickPatch.optional = true;
        waitPickPatch.hasOptionalCost = true;
      ...
      if (/バトンタッチして登場した場合/.test(p)) {
        ...
        if (batonLowSeriesM) waitPickPatch.requiresBatonFromLowerCostSeriesTag = batonLowSeriesM[1];
        var batonExclCharSeriesM = p.match(/「([^」]+)」以外の『([^』]+)』のメンバーからバトンタッチ/);
        if (batonExclCharSeriesM) {
          waitPickPatch.requiresBatonFromSeriesTag = batonExclCharSeriesM[2];
          waitPickPatch.excludeBatonPartnerCharacterName = batonExclCharSeriesM[1];
        }
        var recoverPart = p.split(/登場した場合[,，]?/)[1] || p;
        waitPickPatch.filters = parseAbilityPickFilters(recoverPart);
      }
      if (
        (/手札に加えた場合/.test(p) || /手札に加えた場合/.test(p)) &&
        /登場させてもよ/.test(p)
      ) {
        var optEnterNames = parseQuotedCharacterNames(p);
        if (optEnterNames.length) waitPickPatch.optionalEnterRecoveredNames = optEnterNames;
      }
      return twT(waitPickPatch);
    }
    if (
      ...
      var heartGapTj =
        /元々持つ/.test(p) && /2つ以上少ない/.test(p) && !abilityReferencesPrintedBlade(segRaw);
      return twT({
    ...
    if (td != null && p.includes("控え室")) {
      return twT({
        template: "deck_top_to_waiting",
        deckTopCount: td,
        millBothPlayers: /自分と相手はそれぞれ/.test(p) && /デッキの上から/.test(p),
        requiresOnStage: true,
      });
    }
    ...
    var wtdTj = classifyWaitingPickToDeckPatch(p);
    if (wtdTj) return twT(wtdTj);
    var wunderTj = classifyWaitingMemberUnderStagePatch(p);
    if (wunderTj) return twT(wunderTj);
    var toCenterTj = classifyPickMemberToCenterPatch(p);
    if (toCenterTj) return twT(toCenterTj);
    var lookBotTj = classifyDeckLookReorderPatch(p);
    if (lookBotTj) return twT(lookBotTj);
  ...
  if (enterLiveSuccess) {
    if (!segmentRawOverride && trigger === "live_success") {
      var lssSegRawsMulti = listNativeLiveSuccessSegmentRaws(card);
    ...
    if (/デッキの上からカードを(\d+)枚控え室/.test(p) && /控え室.*ライブカード.*手札/.test(p)) {
      var dwNEarly = p.match(/デッキの上からカードを(\d+)枚控え室/);
      return withTrigger("live_success", {
        template: "live_success_deck_wait_pick_live",
        deckTopCount: dwNEarly ? Number(dwNEarly[1]) : 5,
        filters: parseAbilityPickFilters(p),
        minDistinctLiveNames: /カード名の異なる/.test(p) ? 3 : 0,
      });
    }
    var lsPickPatch = classifyDeckTopPickToHandPatch(p, segRaw);
    if (lsPickPatch) return withTrigger("live_success", lsPickPatch);
    if (
    ...
    var pLsSuccess = stripLiveDrawYellReminderParenthetical(p);
    if (/余剰ハートに/.test(p + segRaw) && /カードを(\d+)枚引/.test(pLsSuccess)) {
      var surplusDrawSlotM = String(segRaw || p).match(/余剰ハートに\{\{heart_0?(\d)[^}]*\}\}/);
      var surplusDrawCntM = pLsSuccess.match(/余剰ハートに[^。]*を([０-９\d]+)つ以上持つ場合/);
      var surplusDrawDrawM = pLsSuccess.match(/カードを(\d+)枚引/);
      if (surplusDrawSlotM && surplusDrawDrawM) {
        return withTrigger("live_success", {
          template: "live_success_surplus_heart_slot_draw",
          surplusHeartSlot: Number(surplusDrawSlotM[1]) || 1,
          minSurplusHeartSlotCount: surplusDrawCntM
            ? Number(normalizeFwDigits(surplusDrawCntM[1])) || 1
            : 1,
          deckDrawCount: Number(surplusDrawDrawM[1]) || 1,
        });
      }
    }
    var lsDraw = pLsSuccess.match(/カードを(\d+)枚引/);
    if (
      lsDraw &&
    ...
    if (
      /相手は余剰ハートをすべて失う/.test(p) &&
      /相手が余剰ハートを2つ以上失っている場合/.test(p) &&
      /このカードのスコアを/.test(normalizeFwDigits(p))
    ) {
      return withTrigger("live_success", {
        template: "live_success_opp_lose_surplus_score",
        cardScoreGrant: parseLiveCardScorePlusFromText(normalizeFwDigits(p)) || 1,
        minOppSurplusHeartsLost: 2,
      });
    }
    if (/余剰ハートを持たない/.test(p) && /余剰ハートを2つ以上/.test(p)) {
    ...
    if (/支払ってもよい/.test(p) && /このカードのスコアを/.test(p) && /つにつき/.test(p)) {
      var perEnergyM = p.match(/支払った[^。]*?([０-９\d]+)つにつき/);
      return withTrigger("live_success", {
        template: "optional_energy_card_score_plus_per_unit",
        optional: true,
        hasOptionalCost: true,
        costEnergy: true,
        costEnergyVariable: true,
        energyUnitsPerScore: perEnergyM ? Number(normalizeFwDigits(perEnergyM[1])) : 4,
        cardScorePerEnergyUnit: parseLiveCardScorePlusFromText(normalizeFwDigits(p)) || 1,
      });
    }
    ...
    var peekOptLsSuccess = classifyDeckTopPeekOptionalWaitPatch(p, {
      optional: true,
      hasOptionalCost: /もよい/.test(p),
      filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p)),
    });
    if (peekOptLsSuccess) {
      return withTrigger("live_success", peekOptLsSuccess);
    }
    ...
    // 必発のミル（例: PL!N-bp7-031「自分のデッキの上からカードを3枚控え室に置く」）
    if (tdOptLs != null && /デッキの上からカードを[０-９\d]+枚控え室に置く/.test(p)) {
      return withTrigger("live_success", {
        template: "deck_top_to_waiting",
        deckTopCount: tdOptLs,
        filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p)),
    ...
    if (/余剰ハート/.test(p) && /エネルギーデッキから.*ウェイト/.test(p)) {
      var surplusSlotSrc = String(segRaw || "") + p;
      var surplusSlotM =
        surplusSlotSrc.match(/余剰ハートに\{\{heart_0?(\d)[^}]*\}\}/) ||
        surplusSlotSrc.match(/余剰ハートに[^、。]*heart_0?(\d)/i);
      return withTrigger("live_success", {
    ...
    var pLsScoreMain = stripLiveDrawYellReminderParenthetical(p);
    var cardScLs = parseLiveCardScorePlusFromText(normalizeFwDigits(pLsScoreMain));
    if (
      /メンバー1人の下にあるすべてのエネルギー/.test(p) &&
      /エネルギー置き場にウェイト/.test(p) &&
      /もよい/.test(p) &&
      cardScLs > 0 &&
      /このカードのスコア/.test(pLsScoreMain)
    ) {
      var minEnAfterBurnM = p.match(/自分のエネルギーが([０-９\d]+)枚以上/);
      return withTrigger("live_success", {
        template: "live_success_under_energy_to_area_score",
        optional: true,
        cardScoreGrant: cardScLs,
        minEnergyCountAfterMove: minEnAfterBurnM ? Number(normalizeFwDigits(minEnAfterBurnM[1])) : 10,
        minMovedEnergyCount: 1,
        requiresOnStage: true,
        filters: parseAbilityPickFilters(""),
      });
    }
    if (
      cardScLs > 0 &&
      /メンバー1人を選ぶ/.test(pLsScoreMain) &&
      /ほかのすべてのメンバーより多くの/.test(pLsScoreMain) &&
      /ブレード/.test(pLsScoreMain + segRaw) &&
      /自分と相手のステージ/.test(pLsScoreMain)
    ) {
      return withTrigger("live_success", {
        template: "live_card_score_plus",
        cardScoreGrant: cardScLs,
        grantPickStageMembersMax: 1,
        requiresStrictlyMostBladesBothStages: true,
        requiresOnStage: true,
        filters: mergeAbilityPickFilters(
    ...
    if (
      cardScLs > 0 &&
      /このカードのスコア/.test(pLsScoreMain) &&
      !isCompoundLiveScoreEffectText(pLsScoreMain) &&
      !/代わり/.test(scoreEffectPartAfterCondition(pLsScoreMain))
    ) {
      /** @type {Partial<ClassifiedAbility>} */
      var lsScorePatch = {
        template: "live_card_score_plus",
      ...
      if (
        ...
        if (/持つ/.test(p)) lsScorePatch.requiresYellRevealedAllBladeHeart = true;
        else lsScorePatch.requiresYellRevealedAllBladeFlipped = true;
      }
      if (
        /エールにより公開/.test(p) &&
        (/ライブカードがある場合/.test(p) ||
          /スコアを持つライブカードが1枚以上/.test(p) ||
          (/を持つライブカードが1枚以上/.test(p) && /icon_score/i.test(String(segRaw || ""))))
      ) {
        lsScorePatch.requiresYellRevealedOwnLiveCard = true;
      }
      if (
        /エールにより公開/.test(p) &&
        /メンバーカードがそれぞれある場合/.test(p) &&
        /heart_0|heart0/i.test(String(segRaw || "") + p)
      ) {
        var yellHeartSlotsLsScore = parseHeartSlotsAnyFromText(p, segRaw);
        if (yellHeartSlotsLsScore.length) {
          lsScorePatch.requiresYellRevealedSeriesHeartSlots = yellHeartSlotsLsScore;
        }
      }
      ...
      if (
        /エールにより公開/.test(p) &&
        /ライブカードが2枚以上あるか/.test(p) &&
        /合計([０-９\d]+)種類以上あるか/.test(p) &&
        /エリアを移動している場合/.test(p)
      ) {
        var orKindsM = p.match(/合計([０-９\d]+)種類以上あるか/);
        /** @type {number[]} */
        var orHeartSlots = [];
        var orSlotRe = /\{\{heart_0?(\d)[^}]*\}\}/gi;
        var orSlotM;
        while ((orSlotM = orSlotRe.exec(String(segRaw || p))) !== null) {
          var orSlot = Number(orSlotM[1]);
          if (orSlot >= 1 && orSlot <= 6 && orHeartSlots.indexOf(orSlot) < 0) orHeartSlots.push(orSlot);
        }
        lsScorePatch.scorePlusOrPreconditions = true;
        lsScorePatch.minYellRevealedLiveCount = 2;
        lsScorePatch.minStageDistinctHeartSlots = orKindsM ? Number(normalizeFwDigits(orKindsM[1])) : 5;
      ...
      applyEnergyAdvantageScoreMeta(lsScorePatch, pLsScoreMain);
      applyYellRevealedAllSeriesScoreMeta(lsScorePatch, pLsScoreMain);
    ...
    if (scoreSetM) {
      ...
      var surplusOrBhM = p.match(/余剰ハートを([０-９\d]+)つ以上持っている場合/);
      if (/ブレードハートを持たないカードが0枚/.test(p) && surplusOrBhM) {
        scoreSetPatch.minSurplusHeartsOrYellAllBh = Number(normalizeFwDigits(surplusOrBhM[1])) || 2;
      } else {
        scoreSetPatch.requiresConditionConfirm = true;
      }
      return withTrigger("live_success", scoreSetPatch);
    }
    // 「エネルギーデッキから、エネルギーカードN枚をこのメンバーの下に置く」
    var edUnderLsSuccess = classifyEnergyDeckUnderMemberPatch(p);
    if (edUnderLsSuccess) return withTrigger("live_success", edUnderLsSuccess);
    var wtdLsSuccess = classifyWaitingPickToDeckPatch(p);
    if (wtdLsSuccess) return withTrigger("live_success", wtdLsSuccess);
    // 「このターン、自分のエネルギーが置き場からエネルギーデッキに置かれていた場合、合計スコア+N」
    var energyReturnScoreLs = parseScorePlusFromText(p);
    if (/エネルギーデッキに置かれていた場合/.test(p) && energyReturnScoreLs > 0) {
      var stageAreaMetaLsSuccess = (function () {
        var areas = parseStageAreaConstraints(segRaw);
        if (areas.length === 1) return { stageArea: areas[0] };
        if (areas.length > 1) return { stageAreas: areas };
        return {};
      ...
      return withTrigger(
        "live_success",
        Object.assign({}, stageAreaMetaLsSuccess, {
          template: "grant_jouji_session",
          liveScoreGrant: energyReturnScoreLs,
          requiresEnergyReturnedToDeckThisTurn: true,
          requiresOnStage: true,
        }),
      );
    }
    // エネルギー操作の汎用形。余剰ハート条件付きなど専用テンプレを先に通すため fallback 直前に置く。
    var edWaitLsSuccess = p.match(/エネルギーデッキから.*エネルギーカードを(\d+)枚ウェイト/);
    if (edWaitLsSuccess) {
      return withTrigger("live_success", {
        template: "energy_deck_to_wait",
        energyWaitCount: Number(edWaitLsSuccess[1]) || 1,
        energySkipNextActivate: textHasEnergySkipNextActivatePhase(p),
        filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p)),
      });
    }
    var actEnLsSuccess = p.match(/エネルギーを(\d+)枚アクティブ/);
    if (actEnLsSuccess && /エネルギーが相手より多い場合/.test(p)) {
      return withTrigger("live_success", {
        template: "activate_energy",
        energyActiveCount: Number(actEnLsSuccess[1]) || 1,
        requiresMoreEnergyThanOpponent: true,
      });
    }
    var liveRetHand = classifyLiveReturnHandDiscardPatch(p);
    if (liveRetHand) return withTrigger("live_success", liveRetHand);
  ...
  if (enterLiveStart) {
    ...
    var pLsMain = stripLiveDrawYellReminderParenthetical(p);
    ...
    if (
      ...
      /メンバーカードを1枚公開/.test(p) &&
      /このメンバーの下に置/.test(p) &&
      textHasHeartColorPickGrant(p)
    ) {
      return lsT(
        enrichHeartColorPickGrantPatch(p, segRaw, {
          template: "live_start_hand_reveal_under_heart_grant",
          requiresOnStage: true,
          optional: true,
          hasOptionalCost: true,
          filters: parseAbilityPickFilters(p),
    ...
    if (textHasHeartColorPickGrant(p)) {
      ...
      var enteredGrant = /このターンに登場したメンバー/.test(p);
      var enteredExclM = p.match(/『([^』]+)』以外のすべてのメンバー/);
      return lsT(
        enrichOptionalHandDiscardCost(
          base,
          enrichHeartColorPickGrantPatch(p, segRaw, {
            template: movedGrant
              ? "live_start_moved_members_pick_heart_grant"
            ...
            grantToEnteredMembersThisTurn: enteredGrant,
            grantExcludeSeriesTag: enteredExclM ? enteredExclM[1] : null,
          }),
          p,
        ),
      );
    ...
    if (perUnitReduce) {
      return lsT(
        Object.assign(perUnitReduce, {
          filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseNeedHeartReduceConditionFilters(p, segRaw)),
        }),
      );
    }

    var millNeedHeartReduce = classifyDeckMillConditionalNeedHeartReduce(p, segRaw);
    if (millNeedHeartReduce) {
      return lsT(millNeedHeartReduce);
    }
    ...
    // 「自分のステージにいる『X』のメンバー1人をアクティブにする」（人数固定・シリーズ条件あり）。
    // E コストが付く形（PL!N-sd2-017）も同じ本体処理で、コストは共通の支払いダイアログが担当する。
    if (/自分のステージにいる(?:ウェイト状態の)?(?:『[^』]+』の)?メンバー1人をアクティブ/.test(p)) {
      return lsT({
        template: "activate_stage_members_up_to",
        activateMax: 1,
        requiresOnStage: true,
        optional: !!base.hasOptionalCost,
        hasOptionalCost: !!base.hasOptionalCost,
        costEnergy: !!base.costEnergy,
        costEnergyCount: base.costEnergy ? base.costEnergyCount || 1 : 0,
        filters: parseAbilityPickFilters(p),
      });
    }
    ...
    if (/以下から1つを選ぶ/.test(p)) {
      var lsChoices = parseAbilityBulletChoices(segRaw);
      var lsPickFilters = mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p));
    ...
    if (
      ...
      var handGrantTotalM = p.match(/合計(\d+)枚/);
      var handGrantCostPart = p.split(/：(?=ライブ終了)/)[0] || p;
      var handGrantNames = parseQuotedCharacterNames(handGrantCostPart);
      if (handGrantNames.length >= 2) {
        return lsT(
          enrichGrantJoujiPatch(p, segRaw, {
            template: "live_start_hand_named_discard_grant_jouji",
            characterNames: handGrantNames,
            handDiscardTotalCount: Number(normalizeFwDigits(handGrantTotalM[1])) || 3,
            optional: true,
            hasOptionalCost: true,
            requiresOnStage: true,
          }),
    ...
    if (/手札の同じユニット名を持つカード(\d+)枚/.test(p) && /ライブ終了時まで/.test(p + segRaw)) {
      var sameUnitM = p.match(/カード(\d+)枚を控え室/);
      return lsT(
        enrichGrantJoujiPatch(p, segRaw, {
          template: "live_start_hand_discard_same_unit_grant",
          handDiscardExact: Number(sameUnitM[1]) || 2,
          optional: true,
          hasOptionalCost: true,
          requiresOnStage: true,
        }),
    ...
    if (/手札の同じグループ名を持つカード(\d+)枚/.test(p) && /ライブ終了時まで/.test(p + segRaw)) {
      var sameGroupM = p.match(/カード(\d+)枚を控え室/);
      return lsT(
        enrichGrantJoujiPatch(p, segRaw, {
          template: "live_start_hand_discard_same_group_grant",
          handDiscardExact: Number(sameGroupM[1]) || 2,
          optional: true,
          hasOptionalCost: true,
          requiresOnStage: true,
        }),
    ...
    if (
      ...
      var maxHandBladeM = p.match(/手札を(\d+)枚まで控え室/);
      var bladeIconsPer = bladeGainFromIcons(segRaw, p) || 1;
      return lsT(
        enrichGrantJoujiPatch(p, segRaw, {
          template: "live_start_hand_discard_optional_blade_per",
          handDiscardMax: Number(maxHandBladeM[1]) || 2,
          bladeGainPerDiscarded: bladeIconsPer,
          optional: true,
          hasOptionalCost: true,
          requiresOnStage: true,
        }),
    ...
    if (
      /手札を(\d+)枚まで控え室/.test(p) &&
      /控え室に置いたカードの枚数に等しい数まで選ぶ/.test(p) &&
      /ライブ終了時まで/.test(p + segRaw) &&
      /ブレード/.test(p + segRaw)
    ) {
      var maxHandBladeEqM = p.match(/手札を(\d+)枚まで控え室/);
      var stageSeriesBladeEqM = p.match(/ステージにいる『([^』]+)』のメンバー/);
      var bladeEqGain = bladeGainFromIcons(segRaw, p) || 1;
      return lsT({
        template: "live_start_hand_discard_optional_blade_pick_equal",
        handDiscardMax: Number(maxHandBladeEqM[1]) || 2,
        bladeGain: bladeEqGain,
        grantToStageSeriesTag: stageSeriesBladeEqM ? stageSeriesBladeEqM[1] : null,
        filters: Object.assign(parseAbilityPickFilters(p), {
          pickType: T_MEMBER,
          seriesTag: stageSeriesBladeEqM ? stageSeriesBladeEqM[1] : null,
        }),
        optional: true,
        hasOptionalCost: true,
        requiresOnStage: true,
    ...
    if (
      /手札の『([^』]+)』のメンバーカードを(\d+)枚まで控え室/.test(p) &&
      /ステージのメンバー1人は/.test(p) &&
      /置いたカード1枚につき/.test(p) &&
      /ライブ終了時まで/.test(p + segRaw)
    ) {
      var seriesHandM = p.match(/手札の『([^』]+)』のメンバーカードを(\d+)枚まで控え室/);
      return lsT(
        enrichGrantJoujiPatch(p, segRaw, {
          template: "live_start_hand_discard_series_member_blade_grant",
          handDiscardMax: Number(seriesHandM[2]) || 3,
          handDiscardSeriesTag: seriesHandM[1],
          bladeGainPerDiscarded: bladeGainFromIcons(segRaw, p) || 1,
          grantPickStageMembersMax: 1,
          filters: Object.assign(parseAbilityPickFilters(p), {
            pickType: T_MEMBER,
            seriesTag: seriesHandM[1],
          }),
          optional: true,
          hasOptionalCost: true,
          requiresOnStage: true,
    ...
    var deckTopMemLs = p.match(/デッキの上からカードを(\d+)枚控え室に置/);
    if (
      deckTopMemLs &&
      /それらがすべて/.test(p) &&
      /メンバーカード/.test(p) &&
    ...
    if (
      ...
      /ブレードハートを持たない/.test(p) &&
      /ライブ終了時まで/.test(p + segRaw) &&
      !/置かれたカード1枚につき/.test(p + segRaw) &&
      !/デッキに置かれたカード1枚につき/.test(p + segRaw)
    ) {
      var wtdBladeIfNoBhLs = classifyWaitingPickToDeckPatch(p);
      if (wtdBladeIfNoBhLs) {
        return lsT(
          Object.assign({}, wtdBladeIfNoBhLs, {
            template: "waiting_to_deck_bottom_blade_if_moved_no_bh",
            bladeGain: bladeGainFromIcons(segRaw, p) || 2,
            optional: true,
            requiresOnStage: true,
          }),
    ...
    if (
      ...
      /デッキの(?:一番)?下/.test(p) &&
      /枚まで/.test(p) &&
      (/置かれたカード1枚につき/.test(p + segRaw) || /デッキに置かれたカード1枚につき/.test(p + segRaw)) &&
      /ライブ終了時まで/.test(p + segRaw)
    ) {
      var wtdBladeLs = classifyWaitingPickToDeckPatch(p);
      if (wtdBladeLs) {
        return lsT(
          Object.assign({}, wtdBladeLs, {
            template: "waiting_to_deck_bottom_blade_per",
            bladeGainPerMoved: bladeGainFromIcons(segRaw, p) || 1,
            requiresOnStage: true,
          }),
        );
      }
    ...
    if (
      ...
      /ブレードハートを持たない/.test(p) &&
      /メンバーカードがある場合/.test(p) &&
      /控え室にあるすべてのカードをシャッフル/.test(p) &&
      /デッキの下に置いてもよい/.test(p) &&
      /ライブ終了時まで/.test(p + segRaw)
    ) {
      var cookSeriesM = p.match(/『([^』]+)』のライブカード/);
      return lsT({
        template: "live_start_optional_shuffle_all_waiting_grant",
        optional: true,
        grantToStageSeriesTag: cookSeriesM ? cookSeriesM[1] : null,
        requiredHeartSlot: 1,
        requiresOnStage: true,
        filters: mergeAbilityPickFilters(parseAbilityPickFilters(p), parseConditionalPrefixFilters(p)),
      });
    ...
    if (/ライブ終了時まで/.test(p + segRaw)) {
      var waitShuffleGrantLs = classifyLiveStartOptionalWaitingShuffleDeckBottomGrant(p, segRaw);
      if (waitShuffleGrantLs) return lsT(waitShuffleGrantLs);
      var millGrantLs = classifyDeckMillConditionalGrantPatch(p, segRaw);
      if (millGrantLs) return lsT(Object.assign({ requiresOnStage: true }, millGrantLs));
      var grantBlade = bladeGainFromIcons(segRaw, p);
      var grantScore = parseScorePlusFromText(p) || parseScorePlusFromText(segRaw.replace(/\{\{[^}]+\}\}/g, ""));
      if (grantBlade > 0 || grantScore > 0 || /を得る/.test(p)) {
        var grantCardScore = parseLiveCardScorePlusFromText(normalizeFwDigits(p)) || 0;
        var lsGrantTpl =
    ...
    var topPickLs = classifyDeckTopPickToHandPatch(p, segRaw, { requiresOnStage: true });
    if (topPickLs) return lsT(topPickLs);

    var lsDraw = pLsMain.match(/カードを(\d+)枚引/);
    if (lsDraw && /相手のステージ.*ウェイト/.test(pLsMain)) {
      var oc2 = pLsMain.match(/コスト(\d+)以下/);
      return lsT({
    ...
    if (
      lsDraw &&
      matchHandTopReorderCount(pLsMain) != null &&
      /ステージのエリアすべて/.test(pLsMain)
    ) {
      var handTopLsM = matchHandTopReorderCount(pLsMain);
      var stageAllSerLsM = pLsMain.match(/ステージのエリアすべてに『([^』]+)』のメンバー/);
      var stageCostSumLsM = pLsMain.match(/コストの合計が(\d+)以上/);
      return lsT({
        template: "draw_then_hand_to_deck_top",
        deckDrawCount: Number(lsDraw[1]) || 1,
        effectHandToDeckTopCount: handTopLsM != null ? handTopLsM : Number(lsDraw[1]) || 1,
        requiresOnStage: true,
        requiresStageAllAreasSeriesTag: stageAllSerLsM ? stageAllSerLsM[1] : null,
        minStageSeriesPrintedCostSum: stageCostSumLsM ? Number(stageCostSumLsM[1]) : null,
    ...
    if (
      ...
      var handTopOptLsM = matchHandTopReorderCount(pLsMain);
      return lsT({
        template: "draw_then_hand_to_deck_top",
        deckDrawCount: Number(lsDraw[1]) || 1,
        effectHandToDeckTopCount: handTopOptLsM != null ? handTopOptLsM : 2,
        optional: true,
        hasOptionalCost: true,
        requiresOnStage: true,
      });
    }
    if (lsDraw && /手札を(\d+)枚デッキの.*?上に置/.test(pLsMain) && !/好きな順番/.test(pLsMain)) {
      var handTopSimpleLsM = pLsMain.match(/手札を(\d+)枚デッキの.*?上に置/);
      return lsT({
        template: "draw_then_hand_to_deck_top",
        deckDrawCount: Number(lsDraw[1]) || 1,
        effectHandToDeckTopCount: handTopSimpleLsM ? Number(handTopSimpleLsM[1]) : 1,
        requiresOnStage: true,
        filters: mergeAbilityPickFilters(parseAbilityPickFilters(pLsMain), parseConditionalPrefixFilters(pLsMain)),
      });
    }
    if (
      lsDraw &&
      /ステージにいるメンバーがすべて『([^』]+)』/.test(pLsMain) &&
      /手札からカードを1枚デッキの一番上か一番下/.test(pLsMain) &&
      /このカードのスコアを/.test(pLsMain)
    ) {
      var allMemSerLs019 = pLsMain.match(/ステージにいるメンバーがすべて『([^』]+)』/);
      return lsT({
        template: "draw_then_hand_to_deck_top",
        deckDrawCount: Number(lsDraw[1]) || 1,
        effectHandToDeckTopCount: 1,
        handToDeckTopOrBottom: true,
    ...
    if (
      lsDraw &&
      !pLsMain.includes("控え室から") &&
      !/手札を\d+枚控え室/.test(pLsMain) &&
      !/このカードのスコア/.test(pLsMain)
    ) {
      return lsT({
        template: "draw_from_deck",
        deckDrawCount: Number(lsDraw[1]) || 1,
        requiresOnStage: true,
        filters: parseAbilityPickFilters(pLsMain),
      });
    }
    ...
    if (
      /メンバー1人につき/.test(p) &&
      /デッキの上からカードを1枚見る/.test(normalizeFwDigits(p)) &&
      /1枚までをデッキの上に置/.test(p) &&
      /その後/.test(p) &&
      /一番上のカードを1枚公開/.test(normalizeFwDigits(p)) &&
      /ライブカードを公開した場合/.test(p) &&
      /このカードのスコア/.test(p)
    ) {
      return lsT({
        template: "live_start_per_series_member_deck_look_reveal_score",
        deckTopCount: 1,
        cardScoreGrant: parseLiveCardScorePlusFromText(normalizeFwDigits(p)) || 1,
        filters: parseAbilityPickFilters(p),
        requiresOnStage: true,
    ...
    var peekOptLs = classifyDeckTopPeekOptionalWaitPatch(p, { requiresOnStage: true });
    if (peekOptLs) return lsT(peekOptLs);
    ...
    if (/自分か相手を選ぶ.*控え室.*デッキの(?:一番)?下/.test(p)) {
      return lsT({
        template: "live_start_pick_player_waiting_deck_bottom",
        deckBottomPickMax: /2枚まで/.test(p) ? 2 : 1,
        filters: { pickType: T_MEMBER },
    ...
    var perHeartColorLs = classifyLiveScorePerHeartColorOnStage(pLsMain, segRaw);
    if (perHeartColorLs) return lsT(Object.assign(perHeartColorLs, { requiresOnStage: true }));

    var namedSlPerM = p.match(/成功ライブカード置き場にあるカード名が「([^」]+)」のカード1枚につき/);
    if (namedSlPerM && /このカードのスコア/.test(pLsMain) && /必要ハート/.test(p) && /増やす/.test(p)) {
      return lsT({
        template: "live_start_score_plus_per_named_success_live",
        namedSuccessLiveCard: namedSlPerM[1],
        cardScorePerUnit: parseLiveCardScorePlusFromText(normalizeFwDigits(pLsMain)) || 1,
        needHeartIncreasePerUnitMap: parseNeedHeartIncreaseFixedMap(segRaw),
        requiresOnStage: true,
      });
    }
    ...
    if (
      ...
      return lsT({
        template: "live_start_score_plus_per_success_live",
        scoreUnitKind: "success_live_cards",
        cardScorePerUnit: parseLiveCardScorePlusFromText(normalizeFwDigits(pLsMain)) || 1,
        needHeartIncreasePerUnitMap: parseNeedHeartIncreaseFixedMap(segRaw),
        requiresOnStage: true,
      });
    ...
    var perUnitScLs = classifyLiveScorePerUnit(pLsMain);
    if (perUnitScLs) return lsT(Object.assign(perUnitScLs, { requiresOnStage: true }));

    var cardScLs = parseLiveCardScorePlusFromText(normalizeFwDigits(pLsMain));
    var pickBladeScoreM =
      pLsMain.match(/メンバー1人を選ぶ。そのメンバーが持つ[^0-9０-９]*([０-９\d]+)つ以上の場合/) ||
      String(segRaw || "").match(/そのメンバーが持つ(?:\{\{icon_blade[^}]*\}\}|ブレード)?(?:が)?([０-９\d]+)つ以上/);
    if (
      cardScLs > 0 &&
      /このカードのスコア/.test(pLsMain) &&
      /メンバー1人を選ぶ/.test(pLsMain) &&
      /そのメンバーが持つ/.test(pLsMain) &&
      pickBladeScoreM
    ) {
      return lsT({
        template: "live_card_score_plus",
    ...
    if (
      cardScLs > 0 &&
      /このカードのスコア/.test(pLsMain) &&
      !isCompoundLiveScoreEffectText(pLsMain) &&
      !/代わり/.test(scoreEffectPartAfterCondition(pLsMain))
    ) {
      /** @type {Partial<ClassifiedAbility>} */
      var lsSuccessScorePatch = {
        template: "live_card_score_plus",
        cardScoreGrant: cardScLs,
        optional: base.optional,
        hasOptionalCost: base.hasOptionalCost,
      ...
      if (
        ...
        (/ライブカードがある場合/.test(pLsMain) ||
          /スコアを持つライブカードが1枚以上/.test(pLsMain) ||
          (/を持つライブカードが1枚以上/.test(pLsMain) && /icon_score/i.test(String(segRaw || ""))))
      ) {
        lsSuccessScorePatch.requiresYellRevealedOwnLiveCard = true;
      }
      if (
        /エールにより公開/.test(pLsMain) &&
        /メンバーカードがそれぞれある場合/.test(pLsMain) &&
        /heart_0|heart0/i.test(String(segRaw || "") + pLsMain)
      ) {
        var yellHeartSlotsLs = parseHeartSlotsAnyFromText(pLsMain, segRaw);
        if (yellHeartSlotsLs.length) {
          lsSuccessScorePatch.requiresYellRevealedSeriesHeartSlots = yellHeartSlotsLs;
        }
      }
      applyEnergyAdvantageScoreMeta(lsSuccessScorePatch, pLsMain);
      applyYellRevealedAllSeriesScoreMeta(lsSuccessScorePatch, pLsMain);
    ...
    // 相手ステージのメンバーをウェイトにする汎用形。登場時（twT 側）と同じ扱いにする。
    // 上のより限定的な分岐（コスト上限のみ・ユニット除外・ステージハート条件など）を通過した残りを拾う。
    if (/相手のステージ/.test(p) && /ウェイト/.test(p)) {
      return lsT(
        Object.assign(buildOppWaitStageMeta(p), {
          optional: /もよい/.test(p),
    ...
    var peekLs = classifyDeckPeekRelocatePatch(p);
    if (peekLs) return lsT(peekLs);
    var lookBotLs = classifyDeckLookReorderPatch(p);
    if (lookBotLs) return lsT(lookBotLs);
    var waitScoreLs = classifyOptionalWaitMembersScorePerPatch(p);
    if (waitScoreLs) return lsT(waitScoreLs);
    var millNamedLs = classifyDeckBottomMillNamedHandPatch(p);
    if (millNamedLs) return lsT(millNamedLs);

    // 「デッキの下からN枚控え室。メンバーがM枚以上ならドロー。すべてメンバーならスコア+」
    var botMillTier = normalizeFwDigits(p).match(
      /デッキの下からカードを(\d+)枚控え室に置/,
    );
    if (
      botMillTier &&
      /メンバーカードが(\d+)枚以上/.test(normalizeFwDigits(p)) &&
      /すべてメンバーカード/.test(p)
    ) {
      var millN = Number(botMillTier[1]) || 5;
      var memTh = normalizeFwDigits(p).match(/メンバーカードが(\d+)枚以上/);
      var drawM = normalizeFwDigits(p).match(/カードを(\d+)枚引/);
      var scoreM = normalizeFwDigits(p).match(/スコアを＋(\d+)/);
      return lsT({
        template: "live_start_deck_bottom_mill_member_tier",
        deckBottomMillCount: millN,
        millMemberDrawThreshold: memTh ? Number(memTh[1]) || 3 : 3,
        deckDrawCount: drawM ? Number(drawM[1]) || 1 : 1,
        cardScoreGrant: scoreM ? Number(scoreM[1]) || 1 : 1,
        requiresOnStage: true,
        filters: parseAbilityPickFilters(p),
      });
    }

    // 「エネルギー1枚をエネルギーデッキに置いてもよい。そうしたとき相手が多いならスコア」
    if (
      /エネルギー([０-９\d]+)?枚をエネルギーデッキに置いてもよ/.test(p) &&
      /相手のエネルギーが自分より/.test(p) &&
      /スコアを＋/.test(p)
    ) {
      var enDeckOpt = p.match(/エネルギー([０-９\d]+)?枚をエネルギーデッキに置いてもよ/);
      return lsT({
        template: "live_start_optional_energy_to_deck_opp_adv_score",
        costEnergyToDeck: true,
        costEnergyToDeckCount: enDeckOpt && enDeckOpt[1] ? Number(normalizeFwDigits(enDeckOpt[1])) || 1 : 1,
        optional: true,
        hasOptionalCost: true,
        requiresOnStage: true,
        filters: parseAbilityPickFilters(p),
    ...
    // 「ライブカード置き場のスコア合計がN以上の場合、合計スコア+1を得る」
    var liveSumGrant = normalizeFwDigits(p).match(
      /ライブカード置き場にあるライブカードのスコアの合計が(\d+)以上の場合/,
    );
    if (liveSumGrant && /合計スコアを＋(\d+)/.test(normalizeFwDigits(p))) {
      var gSc = normalizeFwDigits(p).match(/合計スコアを＋(\d+)/);
      return lsT({
        template: "grant_jouji_session",
        liveScoreGrant: gSc ? Number(gSc[1]) || 1 : 1,
        minLiveAreaScoreSum: Number(liveSumGrant[1]) || 8,
        requiresOnStage: true,
        requiresCenter: parseStageAreaConstraint(segRaw) === "center",
      });
    }
...
export function listNativeLiveSuccessSegmentRaws(card) {
  if (!card || !card.ability) return [];
  var segs = splitAbilityByTriggers(String(card.ability));
  /** @type {string[]} */
  var out = [];
  for (var i = 0; i < segs.length; i++) {
    if (segs[i].trigger !== "live_success") continue;
    var plain = segmentPlainText(segs[i].text);
    if (plain === "/" || plain === "") continue;
    if (plain === "を得る。" || plain === "を得る") continue;
    if (i > 0) {
      var prev = segs[i - 1];
      if (prev.trigger && prev.trigger !== "live_success") {
        var prevText = String(prev.text || "");
        if (/「/.test(prevText) && !/」/.test(prevText) && /ライブ終了時まで/.test(prevText)) continue;
      }
    }
