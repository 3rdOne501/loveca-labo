...
| カード番号 | 名前 | 根因 | 対応 | 横展開 |
|-----------|------|------|------|--------|
| PL!S-bp2-008-R＋ | 小原鞠莉 | (1) `yell_resolution_live_count_score` の `minResolutionLives` が「3枚以上」文言で 3 になり 1〜2枚時+1が死んでいた (2) 付与常時セグメントの tier 判定が全角「＋２」を拾えず 3枚でも+1 | `minResolutionLives:1` + `tierResolutionLives` / `yellRevealLiveCountScoreBonusFromText` | PL!S-bp2-008-P / P＋ / SEC（同 ability・4枚） |
...
| PL!SP-bp7-011-R | 手札全捨てせず6ドロー | `payAbilityCost` ゲートに `costHandDiscardAll` 未列入 | ゲート追加 | `costHandDiscardAll` 全件（PL!SP-bp7-011-P/R・2件） |
| PL!SP-bp7-011-R | Discord 1543837218332868650 再検証（2026-09-01） | 上記修正済（96666a9） | 再現せず | — |
...
| 2026-09-25 | PL!S-bp2-008（R＋/P/P＋/SEC）: エール公開ライブ3枚時の合計スコア+2 — `minResolutionLives` 誤りと付与セグメント tier の全角数字未対応を `yellRevealLiveCountScoreBonusFromText` で横展開（4枚） |
