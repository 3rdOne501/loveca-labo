...
import {
  assemblePublicBoardFromMatchFields,
...
export function mountSimulator(
  ...
  if (versusMatch && versusMatch.spectatorMode) resumeFromStorage = false;
  ...
  if (versusMatch && versusMatch.mode === "local") {
    ...
    var vRole =
      versusMatch.myRole === "spectator"
        ? "spectator"
        : versusMatch.myRole === "guest"
          ? "guest"
          : "host";
    versusSession = {
      mode: "online",
      ...
      opponentName:
        vRole === "spectator"
          ? "観戦"
          : versusOpponentLabel(versusMatch.match, vRole),
  ...
  var isSpectatorSession = !!(versusSession && versusSession.myRole === "spectator");
  if (versusSession) {
    ...
    if (isSpectatorSession) document.body.classList.add("play-spectator-mode");
    ...
    if (concedeBtn) concedeBtn.hidden = isSpectatorSession || versusSession.mode !== "online";
    if (lobbyBtn) lobbyBtn.hidden = versusSession.mode !== "online";
    if (endTurnBtn) endTurnBtn.hidden = isSpectatorSession || versusSession.mode !== "online";
    ...
    var specChrome = document.getElementById("versus-spectator-chrome");
    if (specChrome) specChrome.hidden = !isSpectatorSession;
    var gameBoardEl = root.querySelector(".game-board");
    if (gameBoardEl) gameBoardEl.hidden = isSpectatorSession;
    if (versusSession.mode === "online" && versusMatch && versusMatch.match && !isSpectatorSession) {
  ...
  if (!resumedFromStorage && !isSpectatorSession) {
   ...
   */
  function setEffectProcessingDialogTitle(titleEl, sourceInst, fallbackTitle) {
    if (!titleEl) return;
    if (sourceInst) {
      titleEl.innerHTML = "";
      ...
      titleEl.appendChild(document.createTextNode("の効果処理"));
      return;
    }
    titleEl.textContent = fallbackTitle || "効果の処理";
  }
  ...
  function openMandatoryTriggeredEffectPrompt(item, onDone) {
    ...
    if (title) {
      title.textContent =
        "強制 — " +
        mergedInstDisplayName(inst) +
        "の" +
        abilityKindLabel(kind) +
        "を処理します";
    }
    body.innerHTML =
      '<p class="dlg-ability-guided-plain">' +
  ...
  function checkLocalVersusWinCondition() {
    if (!versusSession || versusSession.mode !== "local" || state.versusMatchEnded) return;
    ...
    var ev = evaluateVersusWinFromCounts({
      hostSuccessLiveCount: mine,
      guestSuccessLiveCount: opp,
    });
    ...
    if (ev.winnerRole === "host") {
      showToast("勝利条件を満たしました！", { duration: 6000 });
    } else {
      showToast("相手の勝利条件を満たしました（負け）", { duration: 6000 });
    }
  }
  ...
  function updateVersusTurnGlow(remoteMatch) {
    ...
    if (phase === "live") {
      ...
      var mine = canRoleActInVersus(m, versusSession.myRole);
    ...
    liveTurnStartGlowArmed = canRoleActInVersus(m, versusSession.myRole);
  }
  ...
  function syncVersusMatchBar(remoteMatch) {
    ...
    if (versusSession.myRole === "spectator") {
      if (turnBadge && m) {
        turnBadge.hidden = false;
        turnBadge.textContent = describeVersusFlowForRole(m, "spectator");
        turnBadge.classList.remove("versus-turn-badge--mine", "versus-turn-badge--opp");
      }
      if (metaEl && m) {
        metaEl.textContent =
          "観戦 · ルーム " +
    ...
    if (
      ...
      var phase = normalizeVersusPhase(m);
      var mineTurn = canRoleActInVersus(m, versusSession.myRole);
      var isLive = phase === "live";
      var flowDesc = describeVersusFlowForRole(m, versusSession.myRole);
      if (turnBadge) {
        turnBadge.hidden = false;
  ...
  function versusBlocksFreePlay() {
    if (versusSession && versusSession.myRole === "spectator") return true;
    if (!versusOnlineActive() || !versusSession.myRole) return false;
    return !canRoleActInVersus(versusSession.remoteMatch, versusSession.myRole);
  }

  var spectatorViewSide = "host";

  function applySpectatorBoardFromRemote(remoteMatch) {
    if (!versusSession || versusSession.myRole !== "spectator" || !remoteMatch) return;
    var pre = spectatorViewSide === "guest" ? "guest" : "host";
    var board = assemblePublicBoardFromMatchFields(
      /** @type {Record<string, unknown>} */ (remoteMatch),
      pre,
    );
    var oppBoardWrap = document.getElementById("versus-opponent-board-wrap");
    if (oppBoardWrap) oppBoardWrap.hidden = false;
    var label =
      (pre === "host" ? remoteMatch.hostName || "ホスト" : remoteMatch.guestName || "ゲスト") +
      " の公開盤面";
    renderVersusOpponentBoard(board, {
      opponentName: label,
      remoteMatch: remoteMatch,
      myRole: "spectator",
    });
    ...
    if (metaEl) {
      metaEl.textContent =
        "観戦 · ルーム " +
        remoteMatch.roomCode +
        " · " +
        describeVersusFlowForRole(remoteMatch, "spectator");
    }
  }
  ...
  function applyVersusLiveJudgmentForMe(remoteMatch) {
    ...
    var iWin = outcome === myRole;
    var iLose =
      (outcome === "hostWin" && myRole === "guest") ||
      (outcome === "guestWin" && myRole === "host");
    ...
    var placedAfter = Array.isArray(state.successfulLiveArea)
      ? state.successfulLiveArea.length
      : 0;
    ...
    if (didPlace && versusSession.roomCode) {
      if (iWin) {
        ...
        var oppBoard = getOpponentBoardPublic(remoteMatch, myRole);
        var oppSlN =
          oppBoard && Array.isArray(oppBoard.successfulLiveArea)
            ? oppBoard.successfulLiveArea.length
            : 0;
      ...
      if (shouldClaimFirst) {
        claimVersusFirstFromSuccessLive(versusSession.roomCode, myRole).catch(function () {
          /* noop */
        });
      }
    }
  ...
  function maybeCommitVersusLiveJudgment(remoteMatch) {
    if (!versusOnlineActive() || !versusSession || versusSession.myRole !== "host") return;
  ...
  function syncVersusPhaseLiveBanner(remoteMatch) {
    ...
    var phase = normalizeVersusPhase(m);
    var mineTurn =
      versusSession.myRole && m && canRoleActInVersus(m, versusSession.myRole);
    var flowDesc = describeVersusFlowForRole(m, versusSession.myRole);
    el.classList.remove(
  ...
  function showVersusFirstPlayerBanner(remoteMatch) {
    ...
    if (!el || !versusSession || !versusSession.myRole || !remoteMatch) return;
    if (remoteMatch.status !== "playing" || !remoteMatch.firstPlayerRole) {
      el.hidden = true;
      return;
    }
  ...
  function doVersusLiveSetDone() {
    ...
    if (getVersusLiveStep(m) !== "set" || !canRoleActInVersus(m, versusSession.myRole)) {
  ...
  function endVersusLiveSetComplete() {
    ...
    reportVersusLiveSetComplete(versusSession.roomCode, versusSession.myRole)
      .then(function () {
        state.liveTurnPickMode = false;
        state.liveTurnSelectedIds = [];
        var m = versusSession.remoteMatch;
        ...
        var mineSecond = !!(second && versusSession.myRole === second);
        if (mineSecond && fp) {
          patchLocalVersusRemoteMatch({
            guestLiveSetDone:
              versusSession.myRole === "guest" ? true : m.guestLiveSetDone,
            hostLiveSetDone:
              versusSession.myRole === "host" ? true : m.hostLiveSetDone,
          });
  ...
  function doVersusLivePerfDone() {
    ...
    var m = versusSession.remoteMatch;
    if (getVersusLiveStep(m) !== "perf" || !canRoleActInVersus(m, versusSession.myRole)) {
      showToast("あなたのパフォーマンス順番ではありません");
      return;
    }
    var perfScore = computeVersusLivePerfScoreTotal();
    reportVersusLivePerfComplete(versusSession.roomCode, versusSession.myRole, perfScore)
      .then(function () {
        resetVersusLocalLiveHandoff();
  ...
  function doVersusSuccessFxDone() {
    if (!versusOnlineActive() || !versusSession.myRole) return;
    var m = versusSession.remoteMatch;
    if (getVersusLiveStep(m) !== "successFx" || !canRoleActInVersus(m, versusSession.myRole)) {
  ...
  function doVersusMulliganExecuteClick() {
    ...
    if (!canRoleActInVersus(m, versusSession.myRole)) {
  ...
  function doVersusEndTurnClick() {
    ...
    endVersusTurn(versusSession.roomCode, versusSession.myRole)
      .then(function () {
        ...
        state.awaitingTurnStart = false;
        liveTurnStartGlowArmed = false;
        if (mBefore && fp && second) {
          if (phaseBefore === "firstNormal" && versusSession.myRole === fp) {
            patchLocalVersusRemoteMatch({
              versusPhase: "secondNormal",
              activePlayerRole: second,
            });
          } else if (phaseBefore === "secondNormal" && versusSession.myRole === second) {
            patchLocalVersusRemoteMatch({
              versusPhase: "live",
              liveStep: "set",
              activePlayerRole: fp,
            ...
            showToast("ライブフェイズへ。先攻はライブカードセットから");
          }
  ...
  function bootVersusOnlineSyncAfterMount() {
    ...
    if (versusSession.myRole === "spectator") {
      ...
      if (versusSession.remoteMatch) {
        applySpectatorBoardFromRemote(versusSession.remoteMatch);
  ...
  function applyRemoteVersusMatch(remoteMatch) {
    ...
    if (versusSession.myRole === "spectator") {
      ...
      if (versusBootstrapping) return;
      applySpectatorBoardFromRemote(remoteMatch);
      syncVersusMatchBar(remoteMatch);
      if (remoteMatch.status === "ended") {
  ...
  function teardownVersusSessionUi() {
    ...
    document.body.classList.remove("play-spectator-mode");
    var versusChromeEl = document.getElementById("versus-play-chrome");
    ...
    if (oppSlWrap) oppSlWrap.hidden = true;
    var specChrome = document.getElementById("versus-spectator-chrome");
    if (specChrome) specChrome.hidden = true;
    var gameBoardEl = root.querySelector(".game-board");
    if (gameBoardEl) gameBoardEl.hidden = false;
    document.body.classList.remove("versus-my-turn", "versus-opponent-turn");
  }
  ...
  function syncPlayFlowHintGlows() {
    ...
    if (versusOnlineActive() && versusSession.remoteMatch) {
      ...
      var vMineAct = canRoleActInVersus(versusSession.remoteMatch, versusSession.myRole);
      body.classList.remove(
        "flow-hint--live-turn-start",
        "flow-hint--live-to-area",
        "flow-hint--live-begin",
  ...
  function syncLiveTurnHandUi() {
    ...
    var versusLive =
      versusOnlineActive() && isVersusLivePhase(versusSession.remoteMatch);
  ...
  function syncMulliganUi() {
    var remoteMatch = versusOnlineActive() ? versusSession.remoteMatch : null;
  ...
  function renderNowImpl() {
    ...
    if (versusOnlineActive() && versusSession.remoteMatch && isVersusLivePhase(versusSession.remoteMatch)) {
  ...
  function doTurnPhaseStart() {
    if (versusOnlineActive() && versusSession.remoteMatch) {
      ...
      if (!canRoleActInVersus(versusSession.remoteMatch, versusSession.myRole)) {
        showToast("あなたのターンではありません");
        return;
      }
    }
  ...
  function doLiveTurnStart() {
    if (state.awaitingTurnStart && !(versusOnlineActive() && isVersusLivePhase(versusSession.remoteMatch))) {
  ...
  document.getElementById("btn-versus-concede")?.addEventListener("click", function () {
    ...
    if (versusSession.mode === "local") {
      state.versusMatchEnded = true;
      showToast("投了しました", { duration: 5000 });
      return;
    }
  ...
  document.getElementById("btn-spectator-view-host")?.addEventListener("click", function () {
    spectatorViewSide = "host";
    var bH = document.getElementById("btn-spectator-view-host");
    var bG = document.getElementById("btn-spectator-view-guest");
    if (bH) bH.setAttribute("aria-pressed", "true");
    if (bG) bG.setAttribute("aria-pressed", "false");
    if (versusSession && versusSession.remoteMatch) applySpectatorBoardFromRemote(versusSession.remoteMatch);
  });
  document.getElementById("btn-spectator-view-guest")?.addEventListener("click", function () {
    spectatorViewSide = "guest";
    var bH = document.getElementById("btn-spectator-view-host");
    var bG = document.getElementById("btn-spectator-view-guest");
    if (bH) bH.setAttribute("aria-pressed", "false");
    if (bG) bG.setAttribute("aria-pressed", "true");
    if (versusSession && versusSession.remoteMatch) applySpectatorBoardFromRemote(versusSession.remoteMatch);
  });
  document.getElementById("btn-versus-lobby-from-spectator")?.addEventListener("click", function () {
    if (versusSession && versusSession.roomCode) {
      persistVersusOnlineSession(versusSession.roomCode, "spectator", false);
    }
    if (typeof onBackToDeck === "function") {
      onBackToDeck({ toVersusLobby: true });
      var dlg = document.getElementById("dlg-versus-lobby");
