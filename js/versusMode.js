 ...
 */
import { getCurrentCloudUser, isCloudSyncAvailable, onCloudUserChange, signInWithGoogle } from "./cloudAuth.js";
...
export function initVersusMode(opts) {
  ...
  function refreshVersusButton() {
    ...
    btnOpen.title =
      "簡易対戦（Firestore 不要）— 相手の成功ライブ枚数を手入力。オンラインルームは Firebase 設定時のみ。";
  ...
  btnOpen?.addEventListener("click", function () {
    ...
    setLobbyStatus(
      "対面・オンラインどちらでも、各プレイヤーが自分の端末で盤面を操作します。勝敗は成功ライブ3枚（相手2枚以下）です。",
    );
    setLobbyRoomCode("");
    updateVersusLobbyButtons(null, null);
    setOnlineSectionVisible();
